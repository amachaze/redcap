...
function loginFunction()
{
	...
	$forgotPassword = "";
	if ($auth_meth_global == "table" || $usingXandTableBasedAuth) {
	...
	if ($auth_meth_global == 'oauth2_azure_ad_table')
	{
		...
		<div class="my-4 fs14">
		    ...
		    <div>
                ...
                <button class="btn btn-sm btn-primaryrc fs15 my-2" onclick="showProgress(1);window.location.href=app_path_webroot_full+'?action=oauth2_azure_ad_login';"><i class="fas fa-sign-in-alt"></i> <?=($GLOBALS['oauth2_azure_ad_name'] != '' ? $GLOBALS['oauth2_azure_ad_name'] : RCView::tt('global_261'))?></button>
	...
	print  "<p style='font-size:13px;'>".RCView::tt_i("config_functions_125", array(
		RCView::a(array(
...
function filter_tags($val, $preserve_allowed_tags=true, $filter_javascript=true, $doRecursive=true, $filter_javascript_without_tags=false)
{
	...
	if ($hasLessThan) {
		...
		if ($filter_javascript_without_tags || ($filter_javascript && strpos($val, '>') !== false))
		{
			...
			do {
				$val = preg_replace($regex, "$1$2$3removed;$5", $val);
    ...
    if ($filter_javascript_without_tags) {
        ...
        $regex = "/(\s*)(javascript\s*:)([^<]*)/i";
        do {
            $val = preg_replace($regex, "$1removed;$3", $val);
        } while (preg_match($regex, $val));
        // Replace any JavaScript events that are NOT used as HTML tag attributes but exist on their own in the string
        $regex = "/(\s*[^a-z]+)(\s+[\/]?src\s*[^\/]*\/)?(on[a-z]+\s*=)([^>]*)/i";
        do {
...
function renderShowProgressDivs()
{
	print	RCView::div(array('id'=>'working'),
				RCView::img(array(
					'data-rc-lang-attrs'=>'alt=design_08',
					'alt'=>RCView::tt_js("design_08"), 
					'src'=>'progress_circle.gif'
				)) . RCView::SP . RCView::tt("design_08")
			) .
...
function mod() {
	...
	if (!is_integer($dividend) || !is_integer($divisor)) return NAN;
	$dividend *= 1;
	$divisor *= 1;
...
function year($val)
{
    if ($val === null || strpos($val, '-') === false) return NAN;
    $parts = explode('-', $val);
...
function concat_ws() {
	...
	$separator = $args[0];
	return join($separator, array_slice($args, 1));
}
...
function linkify($string)
{
    ...
    if (strpos($string, "@") !== false) {
        $string = preg_replace("/([^@\<\>\s]*@[^@\<\>\s]*\.[^@\<\>\s]*)/", '<a href="mailto:$1" class="rc-url-to-link" target="_blank">$1</a>', $string);
    }
    return $string;
}
