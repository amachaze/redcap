...
$(function(){
    ...
    var msgError = "<b><?=js_escape2($lang['check_002']) ?></b> - <?=js_escape2($lang['check_006']) ?> (<?=APP_PATH_SURVEY_FULL?>).";
...
function checkExternalService(serviceUrl, method, divId, msgSuccess, msgError) {
	...
	var thisAjax = $.post(app_path_webroot+'ControlCenter/check_server_ping.php', { url: serviceUrl, type: method }, function(data) {
		if (data.length > 0 && data != 'This URL is not on the restricted list') {
			pass = true;
			resp = msgSuccess;
		}
		$('#'+divId).html(resp);
...
if (!$innodb_file_format_correct) {
    ...
    // Suggest to only use "dynamic" row_format for the InnoDB tables
    $tablesCompact = [];
    ...
    while ($row = db_fetch_array($q)) {
        ...
        if (isset($row2['Row_format']) && strtolower($row2['Row_format']) != 'dynamic') {
