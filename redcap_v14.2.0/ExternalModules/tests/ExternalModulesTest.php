...
class ExternalModulesTest extends BaseTest
{
	...
	function testPerformManagerPageChecks(){
		$assert = function($expectedException = null){
			if($expectedException){
				$this->assertThrowsException(function(){
					ExternalModules::performManagerPageChecks();
		...
		$_SERVER['PHP_SELF'] = parse_url(APP_URL_EXTMOD, PHP_URL_PATH) . 'manager/ajax/some-action.php';
		$assert();
	...
	function testEveryPageTopScript_partial(){
		$assert = function($username, $hasOutput){
			ExternalModules::setUsername($username);
	...
	function testSetSetting_emptyProjectId(){
		foreach([0, null, ''] as $pid){
			$this->assertThrowsException(function() use ($pid){
				$this->callPrivateMethod('setSetting', TEST_MODULE_PREFIX, $pid, TEST_SETTING_KEY, rand());
