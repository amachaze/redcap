$(function(){
	...
	disabledModal.find('.disable-button').click(function(event){
		...
		simpleDialog(
			//= Do you wish to delete the module {0} ({1})? Doing so will permanently remove the module's directory from the REDCap server.	
			ExternalModules.$lang.tt('em_manage_64', '<b>' + title + '</b>', '<b>' + prefix + '_' + version + '</b>'), 
			//= DELETE MODULE?
			ExternalModules.$lang.tt('em_manage_116'),
			null, null, null, 
			//= Cancel
			...
			$.post(url, {prefix: prefix, version: version}, function(data){
				...
				if (errorMessage) {
					if(pid){
						...
						enableErrorDiv.show();
						enableErrorDiv.html(errorMessage + '<br><br><pre>' + jsonAjax['stack_trace'] + '</pre>');
