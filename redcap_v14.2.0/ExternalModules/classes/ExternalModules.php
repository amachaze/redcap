...
class ExternalModules
{
	...
	public static function getLanguageKeyNotDefinedMessage($key, $prefix) {
		if($key === 'em_errors_55' && static::isVanderbilt()){
			global $project_contact_email;
			
			$dev_lang_filename = __DIR__.DS."English.ini";

			$body = [
				defined("APP_PATH_EXTMOD"),
				static::$initialized,
				file_exists($dev_lang_filename),
				parse_ini_file($dev_lang_filename),
				$GLOBALS["lang"]
			];

			$path = tempnam(sys_get_temp_dir(), 'missing-external-module-lang-key-');
			file_put_contents($path, json_encode($body, JSON_PRETTY_PRINT));

			$email = new \Message();
			$email->setFrom($project_contact_email);
			$email->setTo('mark.mcever@vumc.org');
			$email->setSubject('Missing Language Key Details');
			$email->setBody("See $path", true);
			$email->send();
		}
	...
	private static function setSetting($moduleDirectoryPrefix, $projectId, $key, $value, $type = "")
	{
		if(empty($projectId)){
			 ...
			 * https://redcap.vanderbilt.edu/community/post.php?id=222792
			 */
	 ...
	 * @return void
	 */
	static function requireCommandLine(){
		if(!ExternalModules::isCommandLine()){
	...
	public static function isAdminWithModuleInstallPrivileges()
	{
		if(
			 ...
			 * https://redcap.vanderbilt.edu/community/post.php?id=215348&comment=215350
			 */
	 ...
	 * @return void
	 */
	private static function limitDirectFileAccess(){
		if(self::isManagerPage('templates/')){
	 ...
	 * @return void
	 */
	static function performManagerPageChecks(){
		if(self::isManagerPage()){
