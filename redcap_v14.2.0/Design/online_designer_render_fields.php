...
if (count($metadata) > 0) {
	...
	// $action_tags_regex2 = Form::getActionTagMatchRegex();
	...
	foreach ($metadata as $row)
	{
		...
		$fieldContainsEmbeddedFields = true;
		if (!preg_match_all('/(\{)([a-z0-9][_a-z0-9]*)(:icons)?(\})/', "$element_label $element_note $element_enum", $fieldMatches, PREG_PATTERN_ORDER)) {
			$fieldContainsEmbeddedFields = false;
		}
		$fieldContainsEmbeddedFields1 = $fieldContainsEmbeddedFields ? '' : 'hide';
		if ($field_name != $Proj->table_pk) {
			$string_data1 .= ", 'field_embed'=>'<div var=\"$field_name\" class=\"rc-field-embed-designer $fieldIsEmbeddedOnInstrument1 float-end me-1 bg-success text-white\" style=\"padding:1px 3px;\"> <i class=\"fas fa-info-circle\"></i> ".RCView::tt("design_793")."</div>'";
			$string_data1 .= ", 'field_embed_container'=>'<div var=\"$field_name\" class=\"rc-field-embed-parent-designer $fieldContainsEmbeddedFields1 float-end me-1 btn-primaryrc text-white\" style=\"padding:1px 3px;\"> <i class=\"fas fa-info-circle\"></i> ".RCView::tt("design_996")."</div>'";
		}
