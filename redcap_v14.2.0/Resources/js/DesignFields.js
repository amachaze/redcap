...
$(function(){
	...
	$(document).on('mousedown', '.frmedit_tbl', function(event){
		REDCapDesignFields.selectRow(event, this.id);
	});
...
var referenceNode = document.getElementById('design-'+table_pk);
if (elementExists(referenceNode)) {
	var newNode = document.createElement("div");
	newNode.innerHTML = '<div class="frmedit" style="padding:5px 0 10px 5px;margin-bottom:10px;background-color:#DDDDDD;font-size:11px;color:#888;">' + langOD48 + '</div>';
	referenceNode.parentNode.insertBefore(newNode, referenceNode.nextSibling);
}

var REDCapDesignFields = {
	selectRow: function (event,id) {
		var field = id.replace('design-','');
		// Don't do this for Section Headers
		if (id.indexOf('-sh') > -1) return;
		// Don't do this for Matrices
		if ($('tr#'+field+'-tr').hasClass('mtxRow')) return;
		// Don't do this for record ID field
		if (field == table_pk) return;

		if (event.ctrlKey || event.shiftKey || event.metaKey) {
			// Hide popovers being used here that are currently displayed
			$('.frmedit_tbl:not(#'+id+')').popover('dispose');
			//remove element if it's already selected
			if ($('#' + id).hasClass('multiple_fields_selection_var_selected')) {
				$('#' + id).removeClass('multiple_fields_selection_var_selected');
			} else {
				//add more active elements
				$('#' + id).addClass('multiple_fields_selection_var_selected');
				AddTableDrag();
				// Display popover next to field
				$('#' + id).popover({ content: '<div class="mfspc-parent"></div>', title: '<i class="fas fa-wrench"></i> '+langOD54, html: true, placement: 'right', offset: '0 15px' }).popover('show');
				$('.mfspc-parent').html(mfspc).parent().parent().find('.popover-header').css('background-color','#9afeb0');
			}
		} else if (!$(event.target).hasClass("REDCapDesignFieldsOption")) {
			//Clear all selected elements
			$('.frmedit_tbl').removeClass('multiple_fields_selection_var_selected');
			AddTableDrag();
		}
		if (!$('#' + id).hasClass('multiple_fields_selection_var_selected')) {
			$('#' + id).popover('dispose');
			// Add popover to closest highlighted row
			$('.multiple_fields_selection_var_selected:first').popover({ content: '<div class="mfspc-parent"></div>', title: '<i class="fas fa-wrench"></i> '+langOD54, html: true, placement: 'right', offset: '0 15px' }).popover('show');
			$('.mfspc-parent').html(mfspc).parent().parent().find('.popover-header').css('background-color','#9afeb0');
		}
	},
	getSelectRow: function (field) {
		var selectedItems = '';
		$( ".multiple_fields_selection_var_selected" ).each(function( index ) {
			selectedItems += $( this ).attr('id').replace('design-','')+", ";
		});
		if (selectedItems != '') selectedItems = selectedItems.slice(0, -2);
						...
						var saveBranchingLogic = function () {
							...
							$.post(app_path_webroot + 'Design/branching_logic_builder.php?pid=' + pid, {
								...
								if (data.length > 1) {
									...
									if ((data == '3' || data == '4') && super_user_not_impersonator) simpleDialog(langOD49, langOD13);
...
function openAddQuestionBank(sq_id) {
	...
	simpleDialog(null,langOD55,"add_fieldbank",1000, "$('#keyword-search-input').val('');$('#classification-list').val('');");
...
function deleteFieldMulti() {
	deleteField(REDCapDesignFields.getSelectRow(), 0);
}
function deleteField(this_field,section_header,contains_par_info = 0) {
	...
	if (this_field == baseline_date_field) {
		simpleDialog(deleteBaselineDateField, deleteBaselineDateField1);
		return;
	}
	if (section_header) {
		simpleDialog(delSHMsg,delSHTitle,null,null,null,window.lang.global_53,"deleteFieldDo('"+this_field+"',"+section_header+");",langOD24);
	} else {
		var content = delFieldMsg+' "<b>'+this_field+'</b>"'+langQuestionMark;
		if (contains_par_info == 1) {
			content += "<br><br>"+delParInfoFieldWarning;
		}
		simpleDialog(content,delFieldTitle,null,null,null,window.lang.global_53,"deleteFieldDo('"+this_field+"',"+section_header+");",langOD24);
	}
}
...
function deleteFieldDo(this_field,section_header) {
	...
	for (i = 0; i < fields.length; i++) {
		...
		$.post(app_path_webroot + "Design/delete_field.php?pid="+pid, {
			...
			function (data) {
				$('.multiple_fields_selection_var_selected').popover('dispose');
...
function copyFieldMulti() {
	copyField(REDCapDesignFields.getSelectRow());
}
function copyField(this_field) {
	simpleDialog(langOD35+' "<b>'+this_field+'</b>"'+langQuestionMark+' '+langOD36,langOD33,null,null,null,window.lang.global_53,"copyFieldDo('"+this_field+"')",langOD34);
}
//Copy field from a form and add row in Draggable table
var copyFieldsCount = 0;
function copyFieldDo(this_field) {
	var fields = this_field.split(", ");
	var copyFieldsLen = fields.length;
	showProgress(1);
	for (var i = 0; i < copyFieldsLen; i++) {
		var this_field = fields[i];
		$.ajax({
			url: app_path_webroot + "Design/copy_field.php",
			type: 'GET',
			data: {pid: pid, field_name: this_field},
			async: false,
			success:
				function (data) {
					copyFieldsCount++;
					$('.multiple_fields_selection_var_selected').popover('dispose');
					$('.multiple_fields_selection_var_selected').removeClass('multiple_fields_selection_var_selected');
					...
					if ($('#' + this_field + '-tr .headermatrix').length) {
						...
						showProgress(0);
						setTimeout(function () {
							// Reload the table on the last field copied
							if (copyFieldsLen > 1 && copyFieldsCount == copyFieldsLen) {
								reloadDesignTable(getParameterByName('page'));
							}
							// Set autocomplete for BioPortal ontology search for ALL fields on a page
							initAllWebServiceAutoSuggest();
...
function deleteMatrix(current_field,grid_name) {
	...
	simpleDialog(delMatrixMsg+' "'+grid_name+'"'+langQuestionMark+' '+delMatrixMsg2,delMatrixTitle,null,null,null,window.lang.global_53,"deleteMatrixDo('"+current_field+"','"+grid_name+"');",langDelete);
}
...
function moveFieldMulti() {
	moveField(REDCapDesignFields.getSelectRow(),'');
}
function moveField(field,grid_name) {
	...
	$.post(app_path_webroot+'Design/move_field.php?pid='+pid,{ field: JSON.stringify(field), grid_name: grid_name, action: 'view' },function(data){
		var json_data = (typeof(data) == 'object') ? data : JSON.parse(data);
		if (json_data.length < 1) {
			alert(woops);
			return false;
		...
		$('#move_field_popup').dialog({ title: json_data.title, bgiframe: true, modal: true, width: 700, open: function(){fitDialog(this)},
			buttons: [
				...
				{ text: langOD42, click: function () {
					...
					showProgress(1);
					$.post(app_path_webroot+'Design/move_field.php?pid='+pid,{ field: JSON.stringify(field), grid_name: grid_name, action: 'save', move_after_field: $('#move_after_field').val() },function(data){
						$('.multiple_fields_selection_var_selected').popover('dispose');
						if (data == '0') {
							alert(woops);
						} else {
							simpleDialog(data,successfullyMovedMsg,null,600);
						}
						reloadDesignTable(getParameterByName('page'));
						showProgress(0,0);
...
function chkVarFldDisabled(ob) {
	if (status > 0 && $(ob).attr('readonly')) {
		simpleDialog(langOD47,null,'varnameprod-nochange');
	}
}
...
function deleteAttachment() {
	simpleDialog(langOD27, langOD28,null,null,null,window.lang.global_53,
...
function insertRowFromQuestion(tblId, current_field, field_type, this_sq_id, is_last, delete_row) {
	...
	$.get(app_path_webroot+"Design/online_designer_render_fields.php", { pid: pid, page: form_name, field_name: current_field, edit_question: 0, section_header: 0 },
		function(data) {
			...
			highlightTableRow('design-'+current_field,3000);
			//Reset and close popup
			resetAddQuesForm();
			// Initialize all jQuery widgets, etc.
			initWidgets();
