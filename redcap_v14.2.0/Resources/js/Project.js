...
var loadedAceEditorJs = false;
var LogicEditorOpenTime = performance.now();
var LogicEditorCloseTime = performance.now();
function openLogicEditor(opener, fullscreen)
{
    ...
    if (!loadedAceEditorJs) {
        loadJS(app_path_webroot+"Resources/js/Libraries/ace.js");
        loadedAceEditorJs = true;
    }
    // Init settings
    ...
    dlgDiv.dialog({ bgiframe: true, modal: true, width: dialogWidth, title: window.lang.global_170, buttons:
        ...
        {text: buttonText1, click: function() {
            ...
            dlgDiv.remove();
            $('.popover').popover('hide');
            openLogicEditor(opener, !fullscreen);
        }}]
    });
    ...
    dlgDiv.bind('dialogclose', function(){
        // Prevent multiple retriggering of function in small time period
        if ((performance.now()-LogicEditorCloseTime) <= 200) return;
        LogicEditorCloseTime = performance.now();
    ...
    if (!fullscreen && opener.val().trim() == '') {
        var el = $('#rc-ace-editor').popover({ title: window.lang.global_173, content: '<span class="text-secondary fs12">Example: <code>[age] > 30 and [dob] <> ""</code></span>', html: true, placement: 'right', delay: 0, trigger: 'manual' }).popover('show');
        setTimeout(function(){
            $('.popover:visible').css({'top':'-187px', 'z-index':'999'});
        },1);
        setTimeout(function(){
            el.popover('hide');
        },5000);
