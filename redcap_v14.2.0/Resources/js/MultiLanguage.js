 ...
 */
 ML.init = function(data) {
    ...
    $(function() {
        ...
        $('input[data-mlm-config="ui-search"]').on('change keyup paste click search', function(e) {
        ...
        $('input[data-mlm-config="alerts-search"]').on('change keyup paste click search', function(e) {
            const val = ($(e.target).val() ?? '').toString().toLowerCase()
            if (val == '') {
                performAlertsSearch()
            }
...
function updateSwitcher(recreate = false) {
    ...
    switcher.children().each(function() {
        const btn = $(this);
    ...
    addEditLanguageModal = (() => {
        ...
        function handleChanges(e = false) {
            ...
            if (data.mode == 'add-source') {
                ...
                data.uiItems = $modal().find('input[name="include-ui"]').prop('checked') && !data.lang.subscribed; 
...
function generateCSV(data, delimiter) {
    ...
    const sections = ['uiTranslations','uiOptions','fieldTranslations','matrixTranslations','formTranslations','eventTranslations','surveyTranslations','sqTranslations','asiTranslations','alertTranslations','mdcTranslations', 'pdfTranslations', 'protemailTranslations']
...
function updateFormsTable(lang) {
    ...
    for (const formName of Object.keys(config.projMeta.forms)) {
        ...
        $row.find('[data-mlm-type=survey-active]')
            .prop('checked', get_dd(lang, 'survey-active', formName).translation == '1')
        ...
        if (currentLanguage == config.settings.refLang) {
            ...
            // TODO - add indicator for already/not yet translated items
            //$row.find('button[data-mlm-action="translate-formname"]')
        }
        if (form.isSurvey) {
 ...
 * @returns 
 */
function renderFieldItem(lang, meta, template, type, name, prompt) {
    ...
    const emptyRef = isEmpty(meta[type])
    const reference = emptyRef ? $lang.multilang_06 : meta[type].reference
    if (emptyRef) {
        injectDisplayValues($itemTpl, {
            html_prompt: prompt,
            html_reference: reference,
        })
    }
    else {
        injectDisplayValues($itemTpl, {
...
function renderFields() {
    ...
    if (currentLanguage == config.settings.refLang) {
        ...
        for (const strFieldNum of Object.keys(form.fields)) {
            ...
            if(isMatrix && !matrixRendered.includes(matrixName)) {
                ...
                $tpl.find('.mlm-field-items').append(
                    renderFieldItem(
                        ...
                        'matrix-header', matrixName, config.projMeta.fieldTypes['matrix']['header'])
                 )
                // Matrix Enum
                $tpl.find('.mlm-field-items').append(
                    renderFieldEnum(
            ...
            if (!isMatrix && (field["field-header"].reference || !isEmpty(get_dd(lang, 'field-header', fieldName).translation))) {
                $fieldItems.append(renderFieldItem(
                    ...
                    config.projMeta.fieldTypes['header']['header']
                ))
            }
            // Field Label
            if (fieldType.label) {
        ...
        // Update translation status indicators for fields and matrix groups
...
function initializeRichTextEditor(modal, selector, directionality, inverted = false) {
    ...
    const config = {
        ...
        toolbar1: 'blocks bold italic underline forecolor backcolor | link '+fileimageicons+' | fullscreen',
    ...
    if (inverted) {
        ...
        config.content_style = 'body { font-weight: bold; }'
        config.formats = {
            bold: {
                inline: 'span',
                styles: {
...
function set_dd(lang, type, name, index, val, hash = null, update = true) {
    ...
    // Set (new) ref hash in textarea attribute
...
/**
 * Toggles display (collapse/expand) of an alert block based
...
function toggleAlert(alertId, force = null, focus = false) {
    ...
    if (collapsed) {
        ...
        if (focus) $alert.find('input').first().focus()
    }
    else {
        // Collapse
