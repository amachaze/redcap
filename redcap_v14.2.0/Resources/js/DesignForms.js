...
function setUpConditionalInvites(survey_id, event_id, form) {
	...
	$.post(url, { action: 'view' },function(json_data){
		...
		initWidgets();
		// Open dialog
		dialogOb.dialog({ bgiframe: true, modal: true, width: 1330, open:function(){fitDialog(this);}, buttons: [
		...
		for (let j = 0; j < item['form-name'].length; j++) {
			...
			for (let k = i + 1; k < outer_list.length; k++) {
				...
				if (
					...
					simpleDialog(langFDL4 + '<br><br>' + '<b>' + dup, langAlertColon, null, null, null, null, 'saveFormDisplayLogicSettings(true);', null);
					return false;
				}
			}
		}
