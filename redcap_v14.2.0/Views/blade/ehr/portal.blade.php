	...
	<div class="col-12">
		<table class="dataTable cell-border no-footer" style="width:800px;max-width:90%;">
			...
			@foreach ($registeredProjects as $project_id=>$attr)
				@php(isset($rowClass) && $rowClass = ($rowClass == "even") ? 'odd' : 'even')
				<tr class="{{$rowClass}}">
					<td>
					@if($attr['ddp_enabled'])
					...
					@endif
						<div class="float-end" style=""><a class="ehr-remove" href="{{$app_path_webroot}}ehr.php?fhirPatient={{urlencode($patientID)}}&removeProject={{$project_id}}">{{$lang['data_entry_369']}}</a></div>
					</td>
				</tr>
			@endforeach
