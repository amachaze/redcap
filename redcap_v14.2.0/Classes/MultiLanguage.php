 ...
 *   - Only PDFs of individual forms will be output in the chosen language.
 ...
 * 
 * - SPECIAL ATTENTION:
 *   - PDF considerations: see https://redcap.vanderbilt.edu/community/post.php?id=91212
 * 
 ...
 *  - Several strings are defined for JS in init_functions(), such as e.g. langAlert, langOkay, etc.
...
class MultiLanguage
{
    ...
    public static function displaySurveyQueue(Context $context, $survey_ids) {
        ...
        $output = "";
        if ($context->is_ajax) {
            $output .= "<script>REDCap.MultiLanguage.updateSurveyQueue({$json});</script>";
        }
        else {
            // Add JavaScript
    ...
    private static function translateProjectSurveyAccessoryPage(Context $context, $active_langs, $mode) {
        ...
        $allowed = ["survey-title", "survey-acknowledgement", "survey-stop_action_acknowledgement"];
	...
	public static function translationChangesMadeInDraftMode($project_id) {
		$liveMetaHash = sha1(serialize(MultiLanguage::getProjectSettings($project_id, false)));
		$draftMetaHash = sha1(serialize(MultiLanguage::getProjectSettings($project_id, true)));
		return $liveMetaHash !== $draftMetaHash;
	}
    ...
    public static function getTranslationSettings(Context $context, $do_piping, $blocklist = []) {
        ...
        foreach($active_langs as $lang_id) {
            ...
            foreach ($fields as $field_name) {
                ...
                foreach (["field-enum"] as $type) {
                    if ($field_meta["type"] == "yesno") {
                        $field[$type] = array();
                        $field[$type][0] = $this_orig["ui"]["design_99"]["translation"] ?? null; // No
                        $field[$type][1] = $this_orig["ui"]["design_100"]["translation"] ?? null; // Yes
                    }
                    else if ($field_meta["type"] == "truefalse") {
                        $field[$type] = array();
                        $field[$type][0] = $this_orig["ui"]["design_187"]["translation"] ?? null; // False
                        $field[$type][1] = $this_orig["ui"]["design_186"]["translation"] ?? null; // True
                    }
            ...
            foreach ($boolean_overrides as $item) {
                foreach ($item["overrides"] as $id => $override) {
                    $this_lang["ui-translations"][$id] = $override;
                }
            }
            // Add "prepared" language
        ...
        foreach ($fields as $field_name) {
            ...
            foreach(["field-label", "field-note", "field-header"] as $type) {
                ...
                $field[$type] = filter_tags(replaceUrlImgViewEndpoint($reference));
            }
            foreach(["field-video_url"] as $type) {
                $value = $field[$type]["reference"] ?? "";
                if ($value != "") {
    ...
    #region Project Settings and Designated Field
    ...
    public static function save($project_id, $data) {
        try {
            ...
            if ($project_id == "SYSTEM") {
                ...
                $ui_overrides_allowed = !($system_config[""]["disable-ui-overrides"] == "1") || 
                    ($current_config[""]["allow-ui-overrides"] == "1");
                foreach ($data["langs"] as $lang_id => $this_lang) {
                    $current_ui_translations = self::readUITranslations($project_id, $lang_id, $inDraftMode);
    ...
    public static function getProjectMetadata($project_id, $tryDraftMode=false) {
            ...
            if ($is_survey) {
                ...
                $surveys[$name] = array(
                    ...
                    "survey-survey_btn_text_prev_page" => array (
                        ...
                        "prompt" => ensureUTF(RCView::tt_i("multilang_594", [$lang["data_entry_537"]])), //= Translation of the '{0}' button label:
                    ...
                    "survey-survey_btn_text_next_page" => array (
                        ...
                        "prompt" => ensureUTF(RCView::tt_i("multilang_594", [$lang["data_entry_536"]])), //= Translation of the '{0}' button label:
                    ...
                    "survey-survey_btn_text_submit" => array (
                        ...
                        "prompt" => ensureUTF(RCView::tt_i("multilang_594", [$lang["survey_200"]])), //= Translation of the '{0}' button label:
        ...
        #endregion
        // Assemble all into metadata structure
    ...
    public static function getUIMetadata($is_system = false) {
        ...
        $metadata = array (
            ...
            "survey_1130" => [
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["survey_1130"]]), //= The '{0}' tooltip text:
            ],
            "survey_1129" => [
                "category" => "ui-survey",
                "group" => "controls",
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["survey_1129"]]), //= The '{0}' tooltip text:
            ],
            // Language switching
            "multilang_02" => [
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["multilang_02"]]), //= The '{0}' tooltip text:
            ],
            ...
            "data_entry_540" => [
                ...
                "prompt" => RCView::tt_i("multilang_315", [$lang["data_entry_540"]]), //= The '{0}' placeholder text:
            ],
            // Working
            "design_08" => [
                "category" => "ui-common survey dataentry",
                ...
                "prompt" => RCView::tt_i("multilang_336", [$lang["design_08"]]), //= The '{0}' indicator:
            ],
            // Paging and Submit
            "survey_1347" => [
                "category" => "ui-survey",
            ...
            "survey_1559" => [
                ...
                "prompt" => RCView::tt_i("multilang_323", [RCView::tt_attr("survey_1559")]), //= The '{0}' link label:
            ],
            "paging_symbols" => [
                "category" => "ui-survey",
                "group" => "navigation",
                ...
                "id" => "paging_symbols",
                "default" => false,
                "prompt" => RCView::tt_i("multilang_318", array( //= Use {0} and {1} symbols for previous/next buttons?
            ...
            "data_entry_536" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_536"]]), //= The '{0}' button label:
            ],
            "data_entry_537" => [
                "category" => "ui-survey",
                "group" => "navigation",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_537"]]), //= The '{0}' button label:
            ],
            "submit_symbol" => [
                "category" => "ui-survey",
                "group" => "navigation",
                ...
                "id" => "submit_symbol",
                "default" => false,
                "prompt" => RCView::tt_i("multilang_321", array( //= Use the {0} symbol for the submit button instead of a text label?
            ...
            "survey_200" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_200"]]), //= The '{0}' button label:
            ],
            ...
            "form_renderer_19" => [
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["form_renderer_19"]]), //= The '{0}' link label:
            ],
            "form_renderer_20" => [
                "category" => "ui-common survey dataentry",
                "group" => "controls",
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["form_renderer_20"]]), //= The '{0}' link label:
                ],
            "data_entry_39" => [
                "category" => "ui-common survey dataentry",
                "group" => "controls",
                ...
                "prompt" => RCView::tt_i("multilang_336", [$lang["data_entry_39"]]), //= The '{0}' indicator:
            ],
            "form_renderer_66" => [
                "category" => "ui-common survey dataentry",
                "group" => "controls",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["form_renderer_66"]]), //= The '{0}' button label:
            ],
            "calendar_popup_01" => [
                "category" => "ui-common survey dataentry",
                "group" => "dialogs",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["calendar_popup_01"]]), //= The '{0}' button label:
            ],
            "global_53" => [
                "category" => "ui-common survey dataentry",
                "group" => "dialogs",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["global_53"]]), //= The '{0}' button label:
            ],
            "design_654" => [
                "category" => "ui-common survey dataentry",
                "group" => "dialogs",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["design_654"]]), //= The '{0}' button label:
            ],
            "alerts_24" => [
                "category" => "ui-common survey dataentry",
                "group" => "dialogs",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["alerts_24"]]), //= The '{0}' popup window title:
            ],
            "global_01" => [
                "category" => "ui-common",
                "group" => "dialogs",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["global_01"]]), //= The '{0}' popup window title:
            ],
            "global_03" => [
                "category" => "ui-common",
                "group" => "dialogs",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["global_03"]]), //= The '{0}' popup window title:
            ],
            "global_64" => [
                "category" => "ui-common",
                "group" => "dialogs",
            ...
            "survey_1241" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_1241"]]), //= The '{0}' button label:
            ],
            "global_47" => [
                "category" => "ui-common",
                "group" => "misc",
                ...
                "prompt" => RCView::tt_i("multilang_334", [$lang["global_47"]]), //= The alternatives separator '- {0} -' (dashes may be added automatically):
            ],
            "scheduling_35" => [
                "category" => "ui-survey",
                "group" => "print",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["scheduling_35"]]), //= The '{0}' button label:
            ],
            "system_config_623" => [
                "category" => "ui-survey",
                "group" => "print",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["system_config_623"]]), //= The '{0}' button label:
            ],
            "global_143" => [
                "category" => "ui-common",
                "group" => "misc",
                ...
                "prompt" => RCView::tt_i("multilang_336", [$lang["global_143"]]), //= The '{0}' indicator:
            ],
            "global_144" => [
                "category" => "ui-common",
                "group" => "misc",
                ...
                "prompt" => RCView::tt_i("multilang_336", [$lang["global_144"]]), //= The '{0}' indicator:
            ],
            "data_entry_203" => [
                "category" => "ui-common survey dataentry",
                "group" => "misc",
                ...
                "prompt" => RCView::tt_i("multilang_337", [$lang["data_entry_203"]]), //= The '{0}' notification:
            ],
            "data_entry_602" => [
                "category" => "ui-survey",
                "group" => "errors",
            ...
            "docs_1101" => [
                ...
                "prompt" => RCView::tt_i("multilang_333", [$lang["docs_1101"]]), //= The '{0}' hint:
            ],
            "survey_1140" => [
                "category" => "ui-common",
                "group" => "misc",
            ...
            "form_renderer_22" => [
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["form_renderer_22"]]), //= The '{0}' link label:
            ],
            "global_125" => [
                "category" => "ui-common",
                "group" => "controls",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["global_125"]]), //= The '{0}' button label:
            ],
            "data_entry_260" => [
                "category" => "ui-common",
                "group" => "controls",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["data_entry_260"]]), //= The '{0}' prompt:
            ],
            ...
            "data_entry_404" => [
                ...
                "prompt" => RCView::tt_i("multilang_592", [$lang["data_entry_404"]]), //= The '{0}' message (this will be prepended by the actual count):
            ],
            "data_entry_403" => [
                "category" => "ui-common",
                "group" => "controls",
                ...
                "prompt" => RCView::tt_i("multilang_592", [$lang["data_entry_403"]]), //= The '{0}' message (this will be prepended by the actual count):
            ],
            // Checkbox fields
            "data_entry_518" => [
                "category" => "ui-fields",
            ...
            "data_entry_412" => [
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["data_entry_412"]]), //= The '{0}' popup window title:
            ],
            "data_entry_417" => [
                "category" => "ui-fields",
                "group" => "field_checkbox",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_417"]]), //= The '{0}' button label:
            ],
            // Yes/No fields
            "design_100" => [
                "category" => "ui-fields",
                ...
                "prompt" => RCView::tt_i("multilang_316", [$lang["design_100"]]), //= Label for the '{0}' option:
            ],
            "design_99" => [
                "category" => "ui-fields",
                "group" => "field_yesno",
                ...
                "prompt" => RCView::tt_i("multilang_316", [$lang["design_99"]]), //= Label for the '{0}' option:
                ],
            // True/False fields
            "design_186" => [
                "category" => "ui-fields",
                ...
                "prompt" => RCView::tt_i("multilang_316", [$lang["design_186"]]), //= Label for the '{0}' option:
                ],
            "design_187" => [
                "category" => "ui-fields",
                "group" => "field_truefalse",
                ...
                "prompt" => RCView::tt_i("multilang_316", [$lang["design_187"]]), //= Label for the '{0}' option:
                ],
            // Dropdowns
            "data_entry_444" => [
                "category" => "ui-fields",
            ...
            "survey_1142" => [
                ...
                "prompt" => RCView::tt_i("multilang_324", [$lang["survey_1142"]]), //= The '{0}' slider voice explanation:
            ],
            "survey_1143" => [
                "category" => "ui-fields",
                "group" => "field_slider",
                ...
                "prompt" => RCView::tt_i("multilang_324", [$lang["survey_1143"]]), //= The '{0}' slider voice explanation:
            ],
            "survey_1144" => [
                "category" => "ui-fields",
                "group" => "field_slider",
                ...
                "prompt" => RCView::tt_i("multilang_324", [$lang["survey_1144"]]), //= The '{0}' slider voice explanation:
            ],
            "data_entry_609" => [
                "category" => "ui-fields",
                "group" => "field_slider",
            ...
            "dashboard_32" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["dashboard_32"]]), //= The '{0}' button label:
            ],
            "calendar_widget_choosedatehint" => [
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["calendar_widget_choosedatehint"]]), //= The '{0}' tooltip text:
            ],
            "calendar_widget_done" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["calendar_widget_done"]]), //= The '{0}' button label:
            ],
            "calendar_widget_prev" => [
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["calendar_widget_prev"]]), //= The '{0}' tooltip text:
            ],
            "calendar_widget_next" => [
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["calendar_widget_next"]]), //= The '{0}' tooltip text:
            ],
            "form_renderer_29" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["form_renderer_29"]]), //= The '{0}' button label:
            ],
            "calendar_widget_choosedatetimehint" => [
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["calendar_widget_choosedatetimehint"]]), //= The '{0}' tooltip text:
            ],
            "calendar_widget_choosetime" => [
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["calendar_widget_choosetime"]]), //= The '{0}' popup window title:
            ],
            "global_13" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["global_13"]]), //= The '{0}' label:
            ],
            "calendar_widget_hour" => [
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["calendar_widget_hour"]]), //= The '{0}' label:
            ],
            "calendar_widget_min" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["calendar_widget_min"]]), //= The '{0}' label:
            ],
            "calendar_widget_choosetimehint" => [
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["calendar_widget_choosetimehint"]]), //= The '{0}' tooltip text:
            ],
            "calendar_widget_month_day_long_sun" => [
                ...
                "prompt" => RCView::tt_i("multilang_326", [$lang["calendar_widget_month_day_long_sun"]]), //= The '{0}' full day name:
            ],
            "calendar_widget_month_day_short_sun" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_day_long_sun"]]), //= The '{0}' abbreviated day name:
            ],
            "calendar_widget_month_day_long_mon" => [
                ...
                "prompt" => RCView::tt_i("multilang_326", [$lang["calendar_widget_month_day_long_mon"]]), //= The '{0}' full day name:
            ],
            "calendar_widget_month_day_short_mon" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_day_long_mon"]]), //= The '{0}' abbreviated day name:
            ],
            "calendar_widget_month_day_long_tues" => [
                ...
                "prompt" => RCView::tt_i("multilang_326", [$lang["calendar_widget_month_day_long_tues"]]), //= The '{0}' full day name:
            ],
            "calendar_widget_month_day_short_tues" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_day_long_tues"]]), //= The '{0}' abbreviated day name:
            ],
            "calendar_widget_month_day_long_wed" => [
                ...
                "prompt" => RCView::tt_i("multilang_326", [$lang["calendar_widget_month_day_long_wed"]]), //= The '{0}' full day name:
            ],
            "calendar_widget_month_day_short_wed" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_day_long_wed"]]), //= The '{0}' abbreviated day name:
            ],
            "calendar_widget_month_day_long_thurs" => [
                ...
                "prompt" => RCView::tt_i("multilang_326", [$lang["calendar_widget_month_day_long_thurs"]]), //= The '{0}' full day name:
            ],
            "calendar_widget_month_day_short_thurs" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_day_long_thurs"]]), //= The '{0}' abbreviated day name:
            ],
            "calendar_widget_month_day_long_fri" => [
                ...
                "prompt" => RCView::tt_i("multilang_326", [$lang["calendar_widget_month_day_long_fri"]]), //= The '{0}' full day name:
            ],
            "calendar_widget_month_day_short_fri" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_day_long_fri"]]), //= The '{0}' abbreviated day name:
            ],
            "calendar_widget_month_day_long_sat" => [
                ...
                "prompt" => RCView::tt_i("multilang_326", [$lang["calendar_widget_month_day_long_sat"]]), //= The '{0}' full day name:
            ],
            "calendar_widget_month_day_short_sat" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_day_long_sat"]]), //= The '{0}' abbreviated day name:
            ],
            "calendar_widget_month_jan" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_jan"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_feb" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_feb"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_mar" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_mar"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_apr" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_apr"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_may" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_may"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_jun" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_jun"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_jul" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_jul"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_aug" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_aug"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_sep" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_sep"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_oct" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_oct"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_nov" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_nov"]]), //= The '{0}' abbreviated month name:
            ],
            "calendar_widget_month_dec" => [
                ...
                "prompt" => RCView::tt_i("multilang_327", [$lang["calendar_widget_month_dec"]]), //= The '{0}' abbreviated month name:
            ],
            "multilang_108" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_330", [$lang["multilang_108"]]), //= The '{0}' date format indicator:
            ],
            "multilang_109" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_330", [$lang["multilang_109"]]), //= The '{0}' date format indicator:
                ],
            "multilang_110" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_330", [$lang["multilang_110"]]), //= The '{0}' date format indicator:
                ],
            "multilang_111" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_330", [$lang["multilang_111"]]), //= The '{0}' date format indicator:
                ],
            "multilang_112" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_330", [$lang["multilang_112"]]), //= The '{0}' date format indicator:
                ],
            "multilang_113" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_330", [$lang["multilang_113"]]), //= The '{0}' date format indicator:
                ],
            "multilang_114" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_330", [$lang["multilang_114"]]), //= The '{0}' date format indicator:
                ],
            "multilang_115" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_330", [$lang["multilang_115"]]), //= The '{0}' date format indicator:
                ],
            "multilang_116" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_330", [$lang["multilang_116"]]), //= The '{0}' date format indicator:
                ],
            "multilang_117" => [
                "category" => "ui-fields",
                "group" => "field_date",
                ...
                "prompt" => RCView::tt_i("multilang_330", [$lang["multilang_117"]]), //= The '{0}' date format indicator:
                ],
            // File upload fields
            "form_renderer_23" => [
                "category" => "ui-fields",
                ...
                "prompt" => RCView::tt_i("multilang_331", [$lang["form_renderer_23"]]), //= The '{0}' link label and popup title:
                ],
            "data_entry_459" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_331", [$lang["data_entry_459"]]), //= The '{0}' link label and popup title:
                ],
            "form_renderer_24" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["form_renderer_24"]]), //= The '{0}' link label:
            ],
            "form_renderer_25" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["form_renderer_25"]]), //= The '{0}' link label:
                ],
            "data_entry_468" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_333", [$lang["data_entry_468"]]), //= The '{0}' hint:
                ],
            "data_entry_62" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["data_entry_62"]]), //= The '{0}' prompt:
            ],
            "survey_1146" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_333", [$lang["survey_1146"]]), //= The '{0}' hint:
            ],
            "form_renderer_47" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_340", [$lang["form_renderer_47"]]), //= The '{0}' error message:
            ],
            "form_renderer_52" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["form_renderer_52"]]), //= The '{0}' popup window title:
                ],
            "form_renderer_51" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_341", [$lang["form_renderer_51"]]), //= The '{0}' dialog message:
                ],
            "design_397" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["design_397"]]), //= The '{0}' button label:
                ],
            "form_renderer_59" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["form_renderer_59"]]), //= The '{0}' popup window title:
                ],
            "form_renderer_50" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["form_renderer_50"]]), //= The '{0}' prompt:
                ],
            "data_entry_526" => [
                "category" => "ui-fields",
                "group" => "field_upload",
            ...
            "data_entry_64" => [
                ...
                "prompt" => RCView::tt_i("multilang_336", [$lang["data_entry_64"]]), //= The '{0}' indicator:
                ],
            "data_entry_65" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_343", [$lang["data_entry_65"]]), //= The '{0}' message:
            ],
            "data_entry_455" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["data_entry_455"]]), //= The '{0}' popup window title:
                ],
            "data_entry_454" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_341", [$lang["data_entry_454"]]), //= The '{0}' dialog message:
                ],
            "data_entry_453" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["data_entry_453"]]), //= The '{0}' popup window title:
            ],
            "data_entry_460" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_341", [$lang["data_entry_460"]]), //= The '{0}' dialog message:
            ],
            "data_entry_461" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_344", [$lang["data_entry_461"]]), //= The '{0}' notice:
            ],
            "form_renderer_26" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_344", [$lang["form_renderer_26"]]), //= The '{0}' notice:
            ],
            "form_renderer_48" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_340", [$lang["form_renderer_48"]]), //= The '{0}' error message:
                ],
            "form_renderer_49" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_340", [$lang["form_renderer_49"]]), //= The '{0}' error message:
                ],
            "form_renderer_44" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["form_renderer_44"]]), //= The '{0}' popup window title:
                ],
            "form_renderer_45" => [
                "category" => "ui-fields",
                "group" => "field_upload",
            ...
            "form_renderer_60" => [
                ...
                "prompt" => RCView::tt_i("multilang_343", [$lang["form_renderer_60"]]), //= The '{0}' message:
            ],
            "dataqueries_160" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_340", [$lang["dataqueries_160"]]), //= The '{0}' error message:
                ],
            "data_export_tool_248" => [
                ...
                "prompt" => RCView::tt_i("multilang_348", [$lang["data_export_tool_248"]]), //= The '{0}' title (used in PDFs):
            ],
            // Signature fields
            "survey_1147" => [
                "category" => "ui-fields",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["survey_1147"]]), //= The '{0}' label:
            ],
            "form_renderer_31" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_331", [$lang["form_renderer_31"]]), //= The '{0}' link label and popup title:
            ],
            "form_renderer_43" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["form_renderer_43"]]), //= The '{0}' link label:
                ],
            "form_renderer_30" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["form_renderer_30"]]), //= The '{0}' button label:
                ],
            "survey_1147" => [
                "category" => "ui-fields",
                "group" => "field_upload",
            ...
            "form_renderer_46" => [
                ...
                "prompt" => RCView::tt_i("multilang_340", [$lang["form_renderer_46"]]), //= The '{0}' error message:
            ],
            "form_renderer_63" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["form_renderer_63"]]), //= The '{0}' popup window title:
                ],
            "form_renderer_64" => [
                "category" => "ui-fields",
                "group" => "field_upload",
                ...
                "prompt" => RCView::tt_i("multilang_341", [$lang["form_renderer_64"]]), //= The '{0}' dialog message:
                ],
            ...
            "design_205" => [
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["design_205"]]), //= The '{0}' label:
            ],
            "global_290" => [
                "category" => "ui-fields",
                "group" => "field_desc",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["global_290"]]), //= The '{0}' button label:
            ],
            "data_entry_610" => [
                "category" => "ui-fields",
                "group" => "field_desc",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["data_entry_610"]]), //= The '{0}' popup window title:
            ],
            "design_204" => [
                "category" => "ui-fields",
                "group" => "field_desc",
                ...
                "prompt" => RCView::tt_i("multilang_337", [$lang["design_204"]]), //= The '{0}' notification:
            ],
            "global_121" => [
                "category" => "ui-fields",
                "group" => "field_desc",
                ...
                "prompt" => RCView::tt_i("multilang_337", [$lang["global_121"]]), //= The '{0}' notification:
                ],
            // Matrix fields
            "data_entry_204" => [
                "category" => "ui-fields",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["data_entry_204"]]), //= The '{0}' label:
            ],
            // reCAPTCHA
            "survey_1242" => [
                "category" => "ui-captcha",
            ...
            "survey_1243" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_1243"]]), //= The '{0}' button label:
            ],
            "survey_1244" => [
                "category" => "ui-captcha",
                "group" => "captcha",
                ...
                "prompt" => RCView::tt_i("multilang_340", [$lang["survey_1244"]]), //= The '{0}' error message:
            ],
            ...
            "email_users_36" => [
                ...
                "prompt" => RCView::tt_i("multilang_357", [$lang["email_users_36"]]), //= The '{0}' default banner text:
            ],
            "email_users_37" => [
                "category" => "ui-protmail",
                "group" => "protmail_msg",
            ...
            "email_users_38" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["email_users_38"]]), //= The '{0}' button label:
            ],
            "email_users_39" => [
                "category" => "ui-protmail",
                "group" => "protmail_msg",
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["email_users_39"]]), //= The '{0}' link label:
            ],
            "global_234" => [
                "category" => "ui-protmail",
            ...
            "email_users_42" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["email_users_42"]]), //= The '{0}' button label:
            ],
            "email_users_45" => [
                "category" => "ui-protmail",
                "group" => "protmail_codecheck",
                ...
                "prompt" => RCView::tt_i("multilang_333", [$lang["email_users_45"]]), //= The '{0}' hint:
            ],
            "email_users_44" => [
                "category" => "ui-protmail",
                "group" => "protmail_codecheck",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["email_users_44"]]), //= The '{0}' label:
            ],
            "email_users_82" => [
                "category" => "ui-protmail",
                "group" => "protmail_codecheck",
                ...
                "prompt" => RCView::tt_i("multilang_344", [$lang["email_users_82"]]), //= The '{0}' notice:
            ],
            "email_users_101" => [
                "category" => "ui-protmail",
                "group" => "protmail_codecheck",
                ...
                "prompt" => RCView::tt_i("multilang_315", [$lang["email_users_101"]]), //= The '{0}' placeholder text:
            ],
            "email_users_43" => [
                "category" => "ui-protmail",
                "group" => "protmail_codecheck",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["email_users_43"]]), //= The '{0}' button label:
            ],
            "email_users_41" => [
                "category" => "ui-protmail",
                "group" => "protmail_codecheck",
                ...
                "prompt" => RCView::tt_i("multilang_340", [$lang["email_users_41"]]), //= The '{0}' error message:
            ],
            "email_users_50" => [
                "category" => "ui-protmail",
                "group" => "protmail_codecheck",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["email_users_50"]]), //= The '{0}' popup window title:
            ],
            "email_users_35" => [
                "category" => "ui-protmail",
                "group" => "protmail_codecheck",
            ...
            "global_37" => [
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["global_37"]]), //= The '{0}' label:
            ],
            "global_38" => [
                "category" => "ui-protmail",
                "group" => "protmail_display",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["global_38"]]), //= The '{0}' label:
            ],
            "alerts_128" => [
                "category" => "ui-protmail",
                "group" => "protmail_display",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["alerts_128"]]), //= The '{0}' label:
            ],
            "email_users_48" => [
                "category" => "ui-protmail",
                "group" => "protmail_display",
            ...
            "ws_176" => [
                ...
                "prompt" => RCView::tt_i("multilang_368", [$lang["ws_176"]]), //= Email sent time label - '{0}':
                ],
            "ws_177" => [
                "category" => "ui-protmail",
                "group" => "protmail_display",
                ...
                "prompt" => RCView::tt_i("multilang_368", [$lang["ws_177"]]), //= Email sent time label - '{0}':
            ],
            "ws_178" => [
                "category" => "ui-protmail",
                "group" => "protmail_display",
            ...
            "dataqueries_278" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["dataqueries_278"]]), //= The '{0}' button label:
            ],
            "survey_111" => [
                "category" => "ui-survey",
                "group" => "completion",
            ...
            "global_239" => [
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["global_239"]]), //= The '{0}' prompt:
            ],
            "global_240" => [
                "category" => "ui-survey",
                "group" => "login",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["global_240"]]), //= The '{0}' prompt:
            ],
            "survey_573" => [
                "category" => "ui-survey",
                "group" => "login",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["survey_573"]]), //= The '{0}' popup window title:
            ],
            "survey_310" => [
                "category" => "ui-survey",
                "group" => "login",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_310"]]), //= The '{0}' prompt:
            ],
            "survey_574" => [
                "category" => "ui-survey",
                "group" => "login",
            ...
            "survey_1066" => [
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["survey_1066"]]), //= The '{0}' label:
            ],
            "survey_1343" => [
                "category" => "ui-survey",
                "group" => "login",
            ...
            "global_05" => [
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["global_05"]]), //= The '{0}' popup window title:
            ],
            "survey_1353" => [
                "category" => "ui-survey",
                "group" => "login",
            ...
            "config_functions_45" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["config_functions_45"]]), //= The '{0}' button label:
            ],
            "survey_675" => [
                "category" => "ui-survey",
                "group" => "login",
                ...
                "prompt" => RCView::tt_i("multilang_337", [$lang["survey_675"]]), //= The '{0}' notification:
            ],
            "data_entry_262" => [
                "category" => "ui-survey",
                "group" => "misc",
            ...
            "data_export_tool_248" => [
                ...
                "id" => "data_export_tool_248",
                "default" => $lang["data_export_tool_248"], // FILE:
                "prompt" => RCView::tt_i("multilang_336", [$lang["data_export_tool_248"]]), //= The '{0}' indicator:
                ],
            "survey_1365" => [
                "category" => "ui-survey",
                "group" => "pdf",
            ...
            "global_237" => [
                ...
                "prompt" => RCView::tt_i("multilang_344", [$lang["global_237"]]), //= The '{0}' notice:
            ],
            "survey_1139" => [
                "category" => "ui-survey",
                "group" => "pdf",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_1139"]]), //= The '{0}' prompt:
            ],
            "design_121" => [
                "category" => "ui-survey",
                "group" => "pdf",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["design_121"]]), //= The '{0}' button label:
            ],
            "data_entry_535" => [
                "category" => "ui-survey",
                "group" => "pdf",
            ...
            "survey_1264" => [
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["survey_1264"]]), //= The '{0}' popup window title:
            ],
            "survey_1265" => [
                "category" => "ui-survey",
                "group" => "econsent",
            ...
            "survey_1266" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_1266"]]), //= The '{0}' button label:
            ],
            "survey_1364" => [
                "category" => "ui-survey",
                "group" => "econsent",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_1364"]]), //= The '{0}' button label:
            ],
            "survey_1341" => [
                "category" => "ui-survey",
                "group" => "econsent",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_1341"]]), //= The '{0}' button label:
            ],
            "survey_1366" => [
                "category" => "ui-survey",
                "group" => "econsent",
            ...
            "survey_1311" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_1311"]]), //= The '{0}' button label:
            ],
            "survey_1312" => [
                "category" => "ui-survey",
                "group" => "stop",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_1312"]]), //= The '{0}' button label:
            ],
            "data_entry_199" => [
                "category" => "ui-survey",
                "group" => "stop",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["data_entry_199"]]), //= The '{0}' popup window title:
            ],
            "data_entry_265" => [
                "category" => "ui-survey",
                "group" => "stop",
            ...
            "survey_01" => [
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["survey_01"]]), //= The '{0}' popup window title:
            ],
            "survey_02" => [
                "category" => "ui-survey",
                "group" => "stop",
            ...
            "survey_997" => [
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["survey_997"]]), //= The '{0}' tooltip text:
            ],
            "survey_998" => [
                "category" => "ui-survey",
                "group" => "speech",
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["survey_998"]]), //= The '{0}' tooltip text:
            ],
            // Partial Completion
            "survey_163" => [
                "category" => "ui-survey",
            ...
            "control_center_422" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["control_center_422"]]), //= The '{0}' button label:
            ],
            "survey_982" => [
                "category" => "ui-survey",
                "group" => "start",
            ...
            "data_entry_215" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_215"]]), //= The '{0}' button label:
            ],
            "survey_118" => [
                "category" => "ui-survey",
                "group" => "save",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["survey_118"]]), //= The '{0}' label:
            ],
            "survey_657" => [
                "category" => "ui-survey",
                "group" => "save",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_118"]]), //= The '{0}' prompt:
            ],
            "survey_658" => [
                "category" => "ui-survey",
                "group" => "save",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["survey_118"]]), //= The '{0}' popup window title:
            ],
            "survey_659" => [
                "category" => "ui-survey",
                "group" => "save",
                ...
                "prompt" => RCView::tt_i("multilang_341", [$lang["survey_118"]]), //= The '{0}' dialog message:
            ],
            "survey_112" => [
                "category" => "ui-survey",
                "group" => "save",
            ...
            "survey_124" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_124"]]), //= The '{0}' button label:
            ],
            "survey_126" => [
                "category" => "ui-survey",
                "group" => "save",
            ...
            "survey_127" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_127"]]), //= The '{0}' button label:
            ],
            "survey_1339" => [
                "category" => "ui-survey",
                "group" => "save",
            ...
            "survey_1288" => [
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["survey_1288"]]), //= The '{0}' popup window title:
            ],
            "survey_1358" => [
                "category" => "ui-survey",
                "group" => "save",
                ...
                "prompt" => RCView::tt_i("multilang_341", [$lang["survey_1358"]]), //= The '{0}' dialog message:
                ],
            "survey_582" => [
                "category" => "ui-survey",
                "group" => "save",
            ...
            "survey_22" => [
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["survey_22"]]), //= The '{0}' link label:
                ],
            "survey_1141" => [
                "category" => "ui-survey",
                "group" => "save",
            ...
            "survey_23" => [
                ...
                "prompt" => RCView::tt_i("multilang_344", [$lang["survey_23"]]), //= The '{0}' notice:
            ],
            "survey_24" => [
                "category" => "ui-survey",
                "group" => "save",
            ...
            "survey_25" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_25"]]), //= The '{0}' button label:
                ],
            // Survey Return Page
            "survey_661" => [
                "category" => "ui-survey",
            ...
            "survey_662" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_662"]]), //= The '{0}' button label:
                ],
            "survey_110" => [
                "category" => "ui-survey",
                "group" => "save",
            ...
            "survey_515" => [
                ...
                "prompt" => RCView::tt_i("multilang_315", [$lang["survey_515"]]), //= The '{0}' placeholder text:
            ],
            "survey_766" => [
                "category" => "ui-survey",
                "group" => "email",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_766"]]), //= The '{0}' button label:
            ],
            "survey_181" => [
                "category" => "ui-survey",
                "group" => "email",
                ...
                "prompt" => RCView::tt_i("multilang_343", [$lang["survey_181"]]), //= The '{0}' message:
            ],
            "survey_732" => [
                "category" => "ui-survey",
                "group" => "email",
            ...
            "survey_171" => [
                ...
                "prompt" => RCView::tt_i("multilang_450", [$lang["survey_171"]]), //= Translation of '{0}':
            ],
            "survey_173" => [
                "category" => "ui-survey",
                "group" => "results",
                ...
                "prompt" => RCView::tt_i("multilang_450", [$lang["survey_173"]]), //= Translation of '{0}':
            ],
            "survey_176" => [
                "category" => "ui-survey",
                "group" => "results",
                ...
                "prompt" => RCView::tt_i("multilang_450", [$lang["survey_176"]]), //= Translation of '{0}':
            ],
            "survey_210" => [
                "category" => "ui-survey",
                "group" => "results",
            ...
            "survey_178" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_178"]]), //= The '{0}' button label:
                ],
            "survey_179" => [
                "category" => "ui-survey",
                "group" => "results",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_179"]]), //= The '{0}' prompt:
            ],
            "survey_201" => [
                "category" => "ui-survey",
                "group" => "results",
            ...
            "survey_193" => [
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_193"]]), //= The '{0}' prompt:
            ],
            "survey_194" => [
                "category" => "ui-survey",
                "group" => "results",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_194"]]), //= The '{0}' prompt:
            ],
            "survey_195" => [
                "category" => "ui-survey",
                "group" => "results",
            ...
            "locking_25" => [
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["locking_25"]]), //= The '{0}' prompt:
            ],
            "survey_197" => [
                "category" => "ui-survey",
                "group" => "results",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_197"]]), //= The '{0}' prompt:
            ],
            "survey_198" => [
                "category" => "ui-survey",
                "group" => "results",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_198"]]), //= The '{0}' prompt:
            ],
            "survey_199" => [
                "category" => "ui-survey",
                "group" => "results",
            ...
            "survey_509" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_509"]]), //= The '{0}' button label:
            ],
            "dataqueries_23" => [
                "category" => "ui-survey",
                "group" => "queue",
                ...
                "prompt" => RCView::tt_i("multilang_459", [$lang["dataqueries_23"]]), //= The '{0}' column title:
            ],
            "survey_49" => [
                "category" => "ui-survey",
                "group" => "queue",
                ...
                "prompt" => RCView::tt_i("multilang_459", [$lang["survey_49"]]), //= The '{0}' column title:
            ],
            "survey_507" => [
                "category" => "ui-survey",
                "group" => "queue",
                ...
                "prompt" => RCView::tt_i("multilang_336", [$lang["survey_507"]]), //= The '{0}' indicator:
            ],
            "survey_511" => [
                "category" => "ui-survey",
                "group" => "queue",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_511"]]), //= The '{0}' prompt:
            ],
            "survey_504" => [
                "category" => "ui-survey",
                "group" => "queue",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_504"]]), //= The '{0}' button label:
            ],
            "data_entry_174" => [
                "category" => "ui-survey",
                "group" => "queue",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_174"]]), //= The '{0}' button label:
            ],
            "survey_1090" => [
                "category" => "ui-survey",
                "group" => "queue",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_1090"]]), //= The '{0}' button label:
            ],
            "survey_506" => [
                "category" => "ui-survey",
                "group" => "queue",
            ...
            "survey_535" => [
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["survey_535"]]), //= The '{0}' link label:
            ],
            // Survey Queue link
            "survey_512" => [
                "category" => "ui-survey",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_512"]]), //= The '{0}' button label:
            ],
            "survey_510" => [
                "category" => "ui-survey",
                "group" => "queue",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_510"]]), //= The '{0}' button label:
            ],
            ...
            "survey_513" => [
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_513"]]), //= The '{0}' prompt:
                ],
            "global_46" => [
                "category" => "ui-survey",
                "group" => "queue",
            ...
            "survey_514" => [
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_514"]]), //= The '{0}' prompt:
                ],
            "survey_524" => [
                "category" => "ui-survey",
                "group" => "queue",
                ...
                "prompt" => RCView::tt_i("multilang_329", [$lang["survey_524"]]), //= The '{0}' popup window title:
            ],
            "survey_1351" => [
                "category" => "ui-survey",
                "group" => "queue",
            ...
            "survey_522" => [
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["survey_522"]]), //= The '{0}' prompt:
            ],
            "survey_180" => [
                "category" => "ui-survey",
                "group" => "queue",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["survey_180"]]), //= The '{0}' button label:
            ],
            "survey_523" => [
                "category" => "ui-survey",
                "group" => "queue",
            ...
            "form_renderer_18" => [
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["form_renderer_18"]]), //= The '{0}' link label:
            ],
            "data_entry_493" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_locking",
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["data_entry_493"]]), //= The '{0}' prompt:
            ],
            "data_entry_494" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_locking",
            ...
            "data_entry_182" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_182"]]), //= The '{0}' button label:
            ],
            "form_renderer_42" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_locking",
            ...
            "bottom_110" => [
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["bottom_110"]]), //= The '{0}' link label:
            ],
            "bottom_111" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_locking",
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["bottom_111"]]), //= The '{0}' link label:
            ],
            // E-Signature
            "form_renderer_62" => [
                "category" => "ui-dataentry",
            ...
            "form_renderer_21" => [
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["form_renderer_21"]]), //= The '{0}' link label:
            ],
            "data_entry_263" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
            ...
            "survey_413" => [
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["survey_413"]]), //= The '{0}' label
            ],
            "survey_1361" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
            ...
            "survey_649" => [
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["survey_649"]]), //= The '{0}' menu label:
            ],
            "survey_220" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["survey_220"]]), //= The '{0}' menu item label:
            ],
            "bottom_02" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["bottom_02"]]), //= The '{0}' menu item label:
            ],
            "survey_278" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["survey_278"]]), //= The '{0}' menu item label:
            ],
            "survey_628" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["survey_628"]]), //= The '{0}' menu item label:
            ],
            "survey_664" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["survey_664"]]), //= The '{0}' menu item label:
            ],
            "data_entry_181" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["data_entry_181"]]), //= The '{0}' tooltip text:
            ],
            "dataqueries_145" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["dataqueries_145"]]), //= The '{0}' tooltip text:
            ],
            "survey_412" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["survey_412"]]), //= The '{0}' tooltip text:
            ],
            "data_entry_503" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
            ...
            "grid_42" => [
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["grid_42"]]), //= The '{0}' link label:
                ],
            "data_entry_148" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_337", [$lang["data_entry_148"]]), //= The '{0}' notification:
                ],
            "data_entry_149" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_337", [$lang["data_entry_149"]]), //= The '{0}' notification:
                ],
            "data_entry_150" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_337", [$lang["data_entry_150"]]), //= The '{0}' notification:
                ],
            "survey_1350" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["survey_1350"]]), //= The '{0}' link label:
            ],
            "grid_54" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
            ...
            "grid_15" => [
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["grid_15"]]), //= The '{0}' link label:
            ],
            "grid_55" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_misc",
            ...
            "bottom_47" => [
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["bottom_47"]]), //= The '{0}' menu label:
            ],
            "app_24" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_collection_menu",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["app_24"]]), //= The '{0}' menu label:
            ],
            "invite_participants_01" => [
                ...
                "prompt" => RCView::tt_i("multilang_497", [$lang["app_24"]]), //= The '{0}' subtext:
            ],
            "global_91" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_collection_menu",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["global_91"]]), //= The '{0}' menu label:
            ],
            "bottom_60" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_collection_menu",
                ...
                "prompt" => RCView::tt_i("multilang_497", [$lang["global_91"]]), //= The '{0}' subtext:
            ],
            "bottom_62" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_collection_menu",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["bottom_62"]]), //= The '{0}' menu label:
            ],
            "bottom_64" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_collection_menu",
                ...
                "prompt" => RCView::tt_i("multilang_497", [$lang["bottom_62"]]), //= The '{0}' subtext:
            ],
            "bottom_72" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_collection_menu",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["bottom_72"]]), //= The '{0}' menu label:
            ],
            "bottom_73" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_collection_menu",
                ...
                "prompt" => RCView::tt_i("multilang_497", [$lang["bottom_72"]]), //= The '{0}' subtext:
            ],
            "bottom_63" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_collection_menu",
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["bottom_63"]]), //= The '{0}' link label:
            ],
            "global_238" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_collection_menu",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["global_238"]]), //= The '{0}' label:
            ],
            "bottom_23" => [
                "category" => "ui-dataentry",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["bottom_23"]]), //= The '{0}' label:
            ],
            ...
            "edit_project_29" => [
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["edit_project_29"]]), //= The '{0}' label:
            ],
            "data_entry_202" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["data_entry_202"]]), //= The '{0}' menu label:
            ],
            "data_export_tool_158" => [
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["data_export_tool_158"]]), //= The '{0}' menu label:
            ],
            "data_entry_542" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_542"]]), //= The '{0}' menu item label:
            ],
            "data_entry_543" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_543"]]), //= The '{0}' menu item label:
            ],
            "data_entry_544" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_544"]]), //= The '{0}' menu item label:
            ],
            "data_entry_545" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_545"]]), //= The '{0}' menu item label:
            ],
            "data_entry_546" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_546"]]), //= The '{0}' menu item label:
            ],
            "data_entry_599" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
            ...
            "data_entry_547" => [
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_547"]]), //= The '{0}' menu item label:
            ],
            "data_entry_548" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_548"]]), //= The '{0}' menu item label:
            ],
            "data_entry_549" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_549"]]), //= The '{0}' menu item label:
            ],
            "data_entry_556" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_556"]]), //= The '{0}' menu item label:
            ],
            "data_entry_550" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_550"]]), //= The '{0}' menu item label:
            ],
            "data_entry_551" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_551"]]), //= The '{0}' menu item label:
            ],
            "data_entry_552" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_552"]]), //= The '{0}' menu item label:
            ],
            "data_entry_553" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_553"]]), //= The '{0}' menu item label:
            ],
            "data_entry_554" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_554"]]), //= The '{0}' menu item label:
            ],
            "data_entry_555" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["data_entry_555"]]), //= The '{0}' menu item label:
            ],
            "data_entry_557" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["data_entry_557"]]), //= The '{0}' link label:
            ],
            "data_entry_558" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_actions_menu",
            ...
            "grid_51" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["grid_51"]]), //= The '{0}' button label:
            ],
            "data_entry_315" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_record_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["data_entry_315"]]), //= The '{0}' menu label:
            ],
            "data_entry_313" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_record_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["data_entry_313"]]), //= The '{0}' menu label:
            ],
            "data_entry_314" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_record_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["data_entry_314"]]), //= The '{0}' menu label:
            ],
            "data_entry_425" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_record_actions_menu",
            ...
            "data_entry_323" => [
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["data_entry_323"]]), //= The '{0}' menu label:
            ],
            "data_entry_564" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_record_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["data_entry_564"]]), //= The '{0}' menu label:
            ],
            "data_entry_316" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_record_actions_menu",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["data_entry_316"]]), //= The '{0}' menu label:
            ],
            ...
            "data_entry_288" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_288"]]), //= The '{0}' button label:
            ],
            "data_entry_289" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_save",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_289"]]), //= The '{0}' button label:
            ],
            "data_entry_292" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_save",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_292"]]), //= The '{0}' button label:
            ],
            "data_entry_210" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_save",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_210"]]), //= The '{0}' button label:
            ],
            "data_entry_275" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_save",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_275"]]), //= The '{0}' button label:
            ],
            "data_entry_276" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_save",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_276"]]), //= The '{0}' button label:
            ],
            "data_entry_409" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_save",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_409"]]), //= The '{0}' button label:
            ],
            "data_entry_410" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_save",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_410"]]), //= The '{0}' button label:
            ],
            "data_entry_212" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_save",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_212"]]), //= The '{0}' button label:
            ],
            ...
            "data_entry_234" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_234"]]), //= The '{0}' button label:
            ],
            "data_entry_560" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_delete",
            ...
            "data_entry_235" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_235"]]), //= The '{0}' button label:
            ],
            // Change Reason Popup
            "data_entry_603" => [
                "category" => "ui-dataentry",
                ...
                "prompt" => RCView::tt_i("multilang_450", [$lang["data_entry_603"]]), //= Translation of '{0}':
            ],
            "data_entry_68" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_reason",
                ...
                "prompt" => RCView::tt_i("multilang_450", [$lang["data_entry_68"]]), //= Translation of '{0}':
            ],
            "data_entry_69" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_reason",
                ...
                "prompt" => RCView::tt_i("multilang_450", [$lang["data_entry_69"]]), //= Translation of '{0}':
            ],
            "data_entry_70" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_reason",
                ...
                "prompt" => RCView::tt_i("multilang_450", [$lang["data_entry_70"]]), //= Translation of '{0}':
            ],
            "report_builder_28" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_reason",
                ...
                "prompt" => RCView::tt_i("multilang_450", [$lang["report_builder_28"]]), //= Translation of '{0}':
            ],
            ...
            "missing_data_mdc_ni" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['NI']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_inv" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['INV']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_unk" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['UNK']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_nask" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['NASK']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_asku" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['ASKU']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_nav" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['NAV']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_msk" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['MSK']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_na" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_mdc",
                ...
                "prompt" => RCView::tt_i("multilang_522", ['NA']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_navu" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['NAVU']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_np" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_mdc",
                ...
                "prompt" => RCView::tt_i("multilang_522", ['NP']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_qs" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_mdc",
                ...
                "prompt" => RCView::tt_i("multilang_522", ['QS']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_qi" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_mdc",
                ...
                "prompt" => RCView::tt_i("multilang_522", ['QI']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_trc" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['TRC']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_unc" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['UNC']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_der" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['DER']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_pinf" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['PINF']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_ninf" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['NINF']), //= The '{0}' default missing data code label:
            ],
            "missing_data_mdc_oth" => [
                ...
                "prompt" => RCView::tt_i("multilang_522", ['OTH']), //= The '{0}' default missing data code label:
            ],
            "missing_data_04" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_mdc",
                ...
                "prompt" => RCView::tt_i("multilang_325", [$lang["missing_data_04"]]), //= The '{0}' label:
            ],
            "missing_data_03" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_mdc",
                ...
                "prompt" => RCView::tt_i("multilang_312", [$lang["missing_data_03"]]), //= The '{0}' tooltip text:
            ],
            "missing_data_01" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_mdc",
                ...
                "prompt" => RCView::tt_i("multilang_490", [$lang["missing_data_01"]]), //= The '{0}' menu label:
            ],
            "missing_data_02" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_mdc",
                ...
                "prompt" => RCView::tt_i("multilang_491", [$lang["missing_data_02"]]), //= The '{0}' menu item label:
            ],
            ...
            "data_entry_84" => [
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_84"]]), //= The '{0}' button label:
            ],
            "data_entry_85" => [
                "category" => "ui-dataentry",
                "group" => "dataentry_simultaneous",
                ...
                "prompt" => RCView::tt_i("multilang_323", [$lang["data_entry_85"]]), //= The '{0}' link label:
            ],
            ...
            "data_entry_73" => [
                ...
                "prompt" => RCView::tt_i("multilang_313", [$lang["data_entry_73"]]), //= The '{0}' prompt:
            ],
            "data_entry_74" => [
                "category" => "ui-validation",
                "group" => "validation_messages",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_74"]]), //= The '{0}' button label:
            ],
            "data_entry_76" => [
                "category" => "ui-validation",
                "group" => "validation_messages",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["data_entry_76"]]), //= The '{0}' button label:
            ],
            "design_401" => [
                "category" => "ui-validation",
                "group" => "validation_messages",
                ...
                "prompt" => RCView::tt_i("multilang_335", [$lang["design_401"]]), //= The '{0}' button label:
            ],
            "data_entry_423" => [
                "category" => "ui-validation",
                "group" => "validation_messages",
            ...
            "data_entry_105" => [
                ...
                "prompt" => RCView::tt_i("multilang_587", [$lang["data_entry_105"]]), //= The {0} popup title:
            ],
            "data_entry_575" => [
                "category" => "ui-validation",
                "group" => "validation_secondary",
        ...
        foreach ($validations as $val_name => $val_info) {
            ...
            $metadata[$val_id] = array(
                ...
                "prompt" => RCView::tt_i("multilang_577", [
                    $visible,
                    $val_info["label"],
    ...
    public static function getExportFile($project_id, $user, $payload) {
        ...
        $content["instructions"] = $request["format"] == "json"
            // JSON instructions
            ? (RCView::getLangStringByKey("multilang_137") . ($project_id == "SYSTEM" ? "" : (" ".RCView::getLangStringByKey("multilang_138"))))
            // CSV instructions 
    ...
    private static function checkTypeNameIndex($pmd, $pmd_map, $type, $name, $index) {
        ...
        if (!in_array($type, self::$allowedMetaDataTypes)) return false;
     ...
     */
    private static function readConfig($project_id, $lang=NULL, $tryDraftMode=false) {
        if ($project_id == "SYSTEM") {
