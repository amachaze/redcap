...
class FileRepository
{
    ...
    public static function getBreadcrumbs($folder_id, $type, $recycle_bin=0)
    {
        ...
        elseif ($type == 'pdf_archive') {
            ...
            return "<div class='ItemListBreadcrumbSeparator'>/</div><div class='ItemListBreadcrumb'>".RCView::tt('survey_1160')."</div>$downloadBtn";
        }
        // Data Export files
        elseif ($type == 'export') {
    ...
    public static function getFileList($folder_id=null, $type=null, $recycle_bin=0)
	{
        ...
        if (!$pdfAutoArchiverEnabled) {
            $sql = "select count(*) from redcap_surveys_pdf_archive a, redcap_edocs_metadata e
                    ...
                    and a.survey_id in (".prep_implode(array_keys($Proj->surveys)).")
                    and e.delete_date is null and e.project_id = " . $Proj->project_id;
            $pdfAutoArchiverEnabled = (db_result(db_query($sql), 0) > 0);
        }
        ...
        if ($type == null)
        {
            ...
            if ($folder_id == null && $recycle_bin == 0)
            {
                ...
                if ($pdfAutoArchiverEnabled) {
                    ...
                    $thisFileCount = count(Survey::getPdfAutoArchiveFiles($Proj, (isset($user_rights['group_id']) ? $user_rights['group_id'] : null)));
                    $thisFileCountText = RCView::span(['class'=>'nowrap'], $thisFileCount . " " . ($thisFileCount === 1 ? $lang['docs_76'] : $lang['api_99']));
                    $rows[] = [0 => $topLevelFolderIcon1, 1 => "$topLevelFolderIcon2 <a href='javascript:;' class='fs14' onclick=\"loadFileRepoTable('','pdf_archive',0);\">{$lang['survey_1160']}</a>", 2 => ['sort'=>$thisFileCount, 'display'=>$thisFileCountText], 3 => ['sort'=>"", 'display'=>""], 4 => "", 5 => "", 6 => "", 7 => ""];
                }
                if ($recordLockingPdfEnabled) {
            ...
            if ($file_repository_enabled)
            {
                ...
                if (!FileRepository::userHasFolderAccess($folder_id) || $noDagAccess) {
                    ...
                    while ($row = db_fetch_row($q)) {
                        ...
                        if (!$recycle_bin) {
                            // Share icons
                            $row[5] = "<a href='javascript:;' class='fs15 opacity65 px-2' title='" . RCView::tt_js('form_renderer_65') . "' onclick=\"fileRepoGetPublicLink($doc_id,'".htmlspecialchars(strip_tags($filename), ENT_QUOTES)."');\"><i class='fa-solid fa-arrow-up-from-bracket'></i></a>";
                            // Delete icon
        ...
        elseif ($type == 'pdf_archive')
        {
            ...
            foreach ($files as $file)
            {
                ...
                $isEconsentPdf = ($Proj->surveys[$file['survey_id']]['pdf_auto_archive'] == '2' || trim($file['identifier'].$file['version'].$file['type']) != '');
                // Secondary ID / CRL
                $record_extra_label = isset($extra_record_labels[$file['record']]) ? " ".$extra_record_labels[$file['record']] : "";
                ...
                // Filename
                $filename = $file['doc_name'];
                if ($user_rights['forms_export'][$Proj->surveys[$file['survey_id']]['form_name']] == '1') {
                ...
                $row[] = $filename;
                // Record name
                $row[] = RCView::a(['style'=>'text-decoration:underline;padding:0 5px;font-size:14px;', 'title'=>$lang['docs_96'], 'href'=>APP_PATH_WEBROOT . "DataEntry/index.php?pid=".$Proj->project_id."&instance={$file['instance']}&event_id={$file['event_id']}&page={$Proj->surveys[$file['survey_id']]['form_name']}&id={$file['record']}"],
                    $file['record'] . " $record_extra_label"
                );
                // Survey
                $row[] = $Proj->surveys[$file['survey_id']]['title']
                        . ($Proj->longitudinal ? " (".$Proj->eventInfo[$file['event_id']]['name_ext'].")" : "")
                        . ($hasRepeatingFormsEvents ? " #".($isRepeatingFormOrEvent ? $file['instance'] : "") : "");
    ...
    public static function renderIndexPage()
    {
        ...
        addLangToJS(['period', 'docs_74', 'sendit_32', 'docs_77', 'docs_78', 'docs_79', 'docs_80', 'docs_81', 'docs_82', 'sendit_03', 'sendit_04', 'sendit_05', 'global_01',
            ...
            'docs_92', 'docs_93', 'docs_94', 'global_49', 'survey_437', 'survey_1161', 'survey_1172', 'survey_1221', 'survey_1173', 'survey_1174', 'docs_97', 'docs_98', 'docs_99',
            ...
            'docs_1145', 'docs_1146', 'docs_1147', 'global_03', 'docs_1149', 'docs_1150', 'docs_1152', 'docs_1153']);
    ...
    public static function renderFileDownloadPage($fileAttr)
    {
        ...
        // Display PDF
        elseif ($fileExt == 'pdf') {
            $image = "<div class='pt-3'><div class='text-secondary fs16 mb-2'>{$lang['docs_1100']}</div><object data='$image_view_page' type='application/pdf' style='width:100%;max-width:100%;height:800px;'><iframe src='$image_view_page' style='width:100%;max-width:100%;height:300px;border:none;'></iframe></object></div>";
        }
