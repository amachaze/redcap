            ...
            if (!ParticipantHandler::isDateFormatValid(
                ...
                if ($joinDate == null || strlen($joinDate) == 0) {
                    ...
                    if ($flag == false) {
                        ...
                        // Update Install Date (if applicable) - Update field value where action tag is @MC-FIELD-PARTICIPANT-INFO(join_date)
                            ...
                            if ($attr['redcap_repeat_instance'] == $instance || $attr['redcap_repeat_instance'] == "") {
                                foreach ($fields as $field) {
                                    if ($field != $Proj->table_pk) { // Skip when its primary key
                                        $newInstallDate = $data['joindate'];
                                        // Save install date value
                                        $record_data = [[$Proj->table_pk => $record, $field => $newInstallDate]];
