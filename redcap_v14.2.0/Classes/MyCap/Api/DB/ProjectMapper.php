 ...
 *
 * @see https://redcap.vanderbilt.edu/redcap_v7.0.0/Plugins/index.php REDCap Developer Tools
 * @package REDCapExt
 */
class ProjectMapper
{
    ...
    /** @var string|null Repeating instance name. Form/instrument name */
    public $nonRepeatInstanceName = null;
    ...
    /**
     * Get display friendly name of an instrument given it's form name
     *
     * @see /redcap_vX.X.X/Plugins/index.php?REDCapMethod=getInstrumentNames
     * @param $formName
     * @return string
     */
    public static function instrumentName($formName)
    {
        return \REDCap::getInstrumentNames($formName);
    }
    ...
    public function oneByFilter($filter = null)
    {
        if ($this->longitudinal) {
            if ($this->isRepeatingInstrument()) {
                throw new \Exception("This method can only be used by non-repeating instruments");
            }
        } else {
            if (!$this->isRepeatingInstrument()) {
    ...
    public function fieldMap($forceRefresh = false)
    {
        ...
        foreach ($dictionaryFields as $name => $field) {
            if (in_array(
                ...
                if ($this->longitudinal) {
                    if ($field['form_name'] != $this->nonRepeatInstanceName) {
                        continue;
                    }
                } else {
    ...
    public function nextId()
    {
        ...
        if (!$this->longitudinal) {
            if ($this->isRepeatingInstrument()) {
                ...
                $nextId = \RepeatInstance::getNextRepeatingInstance($this->pid, $this->repeatRecordId, $Proj->firstEventId, $this->repeatInstanceName);
            } else {
    ...
    {
        if ($this->longitudinal) {
            $completeField = $this->nonRepeatInstanceName . '_complete';
        } else {
