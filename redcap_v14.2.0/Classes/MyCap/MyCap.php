...
class MyCap
{
    ...
    public function loadMyCapProjectValues()
    {
        ...
        $sql = "SELECT * FROM redcap_mycap_tasks WHERE project_id = " . $this->project_id." AND enabled_for_mycap = 1";
        $q = db_query($sql);
        $tasks_order = array();
        while ($row = db_fetch_assoc($q))
        {
    ...
    public function generateProjectConfigJSON($projectId = null, $publishedFrom = 'all') {
        ...
        if ($publishedFrom == 'mycaptasks' || $publishedFrom == 'all') {
            ...
            foreach ($tasks as $taskId => $task) {
                ...
                if (!$Proj->longitudinal || !empty($taskSchedules)) {
                    ...
                    $tasksData[$i]['allowsSaving'] = ($task['allow_save_complete_later'] == 1) ? true : false;
                    $tasksData[$i]['allowsRetroactiveCompletion'] = ($task['allow_retro_completion'] == 1) ? true : false;
                    $tasksData[$i]['extendedConfigJson'] = (!is_null($task['extended_config_json'])) ? $task['extended_config_json'] : "";
                    $tasksData[$i]['sortOrder'] = $Proj->forms[$form_name]['form_number'];
                    $tasksData[$i]['notification'] = "";

                    $type = $task['schedule_type'];
                    $frequency = ($task['schedule_type'] == Task::TYPE_REPEATING) ? $task['schedule_frequency'] : null;
                    $interval = 0;
                    if ($frequency == Task::FREQ_WEEKLY) {
                        $interval = $task['schedule_interval_week'];
                    } else if ($frequency == Task::FREQ_MONTHLY) {
                        $interval = $task['schedule_interval_month'];
                    }

                    if ($Proj->longitudinal) {
                        $taskSchedules = Task::getTaskSchedules($taskId);
                        if (count($taskSchedules) > 0) {
                            foreach ($taskSchedules as $eventId) {
                                $tasksData[$i]['longitudinalSchedule'][] = array('identifier' => $eventId,
                                    'arm' => $Proj->eventInfo[$eventId]['arm_num'],
                                    'type' => Task::TYPE_ONETIME,
                                    'frequency' => $frequency,
                                    'interval' => (int)$interval,
                                    'daysFixed' => ($task['schedule_days_fixed'] != "") ? $task['schedule_days_fixed'] : null,
                                    'daysOfTheMonth' => ($task['schedule_days_of_the_month'] != "") ? $task['schedule_days_of_the_month'] : null,
                                    'daysOfTheWeek' => ($task['schedule_days_of_the_week'] != "") ? $task['schedule_days_of_the_week'] : null,
                                    'relativeTo' => ($task['schedule_relative_to'] != "") ? $task['schedule_relative_to'] : null,
                                    'relativeOffset' => (int) $Proj->eventInfo[$eventId]['day_offset'],
                                    'ends' => ($task['schedule_ends'] != "") ? $task['schedule_ends'] : Task::ENDS_NEVER,
                                    'endAfterDays' => (int)$task['schedule_end_after_days'],
                                    'endCount' => (int)$task['schedule_end_count'],
                                    'endDate' => ($task['schedule_end_date'] != "") ? $task['schedule_end_date'] : null);
                            }
                        }
                    } else {
                        $tasksData[$i]['schedule'] = array('type' => $type,
                            ...
                            'daysOfTheWeek' => ($task['schedule_days_of_the_week'] != "") ? $task['schedule_days_of_the_week'] : null,
                            'relativeTo' => ($task['schedule_relative_to'] != "") ? $task['schedule_relative_to'] : null,
                            'relativeOffset' => (int)$task['schedule_relative_offset'],
                            'ends' => ($task['schedule_ends'] != "") ? $task['schedule_ends'] : Task::ENDS_NEVER,
                            'endAfterDays' => (int)$task['schedule_end_after_days'],
                            'endCount' => (int)$task['schedule_end_count'],
                            'endDate' => ($task['schedule_end_date'] != "") ? $task['schedule_end_date'] : null);
                ...
                if ($task['card_display'] == Task::TYPE_DATELINE) {
                    ...
                    if ($task['include_instruction_step'] == 1) {
                        ...
                        $tasksData[$i]['instructionStep']['title'] = $task['instruction_step_title'];
                        $tasksData[$i]['instructionStep']['content'] = $task['instruction_step_content'];
                    ...
                    if ($task['include_completion_step'] == 1) {
                        ...
                        $tasksData[$i]['completionStep']['title'] = $task['completion_step_title'];
                        $tasksData[$i]['completionStep']['content'] = $task['completion_step_content'];
                    ...
                    if ($isPromis) {
                        ...
                        $batteryInstrumentsList = Task::batteryInstrumentPositions();
     ...
     * @return array
     */
    public static function getFileNames($fil_category, $projectId) {
        $values = array();
        switch ($fil_category) {
     ...
     * @param void
     */
    public function copyProjectParticipantSettings($copyof_project_id, $new_project_id) {
        ...
        while ($row = db_fetch_assoc($q))
        {
            $sql = "UPDATE redcap_mycap_projects SET allow_new_participants = '".$row['allow_new_participants']."', participant_custom_field = '".$row['participant_custom_field']."',
                            ...
                            baseline_date_field = '".$row['baseline_date_field']."', baseline_date_config = '".db_escape($row['baseline_date_config'])."',
                            status = '".$row['status']."'
    ...
    public static function getMyCapSetupInstructions() {
        ...
        $text .= RCView::div(array('style'=>'font-size:13px;font-weight:bold;margin:15px 0 2px;'), $lang['mycap_mobile_app_550']) .
                RCView::div(array('style'=>''), $lang['mycap_mobile_app_551']);
        ...
        $text .= RCView::div(array('style'=>'font-size:13px;font-weight:bold;margin:15px 0 2px;'), $lang['mycap_mobile_app_646']) .
                RCView::div(array('style'=>''), $lang['mycap_mobile_app_647']);
					...
					RCView::div(array('class' => 'mt-4 py-2 px-3', 'style'=>'background-color:#f5f5f5;border:1px solid #ddd;'),
						RCView::div(array('class' => 'mb-3'),
							...
							RCView::ul(['class' => 'mb-0'],
								...
								RCView::li(['class' => 'mt-1'], RCView::a(['href'=>'javascript:;', 'onclick'=>"popupvid('mycap_02.mp4','".RCView::tt_js('mycap_mobile_app_623')."');", 'target' => '_blank', 'style' => 'text-decoration:underline'], '<i class="fa-solid fa-film me-1"></i>' . $lang['mycap_mobile_app_623'])) .
