...
class NIHQuestionLibrary extends FieldBank
{
     ...
     */
    public function getSingleCDE($tinyId, $ticket = '')
    {
            ...
            if ($noOfPVs > 0) {
                ...
                if ($isLatestCache == false || $isPartialPVs == true) {
                    ...
                    if ($replace_pvs == true || $isPartialPVs == true) {
                        $ticket = '';
                        // generate ticket only if PVs contains "login to see.." text otherwise pass ticket as blank to singlecde call
                        if ($replace_pvs == true) {
                            // Else, make NLM API call and get choices from response
                            // Retrieves a single-use service ticket for the UMLS REST API
                            $this->generateTGIApiUrl();
                            $ticket = $this->generateUTSServiceTicket();
                        }
                        // Pass ticket as blank if choices are truncated in response to get full list
                        $result = $this->getSingleCDE($tinyId, $ticket);
                        $element_enum = [];
                        $number = 0;
                        ...
                        foreach ($iterateArray as $k => $thisPV) {
                            ...
                            $array['cdes'][$key]['valueDomain']['permissibleValues'][$k]['valueMeaningName'] = isset($result['valueDomain']['permissibleValues'][$k]['valueMeaningName']) ? $result['valueDomain']['permissibleValues'][$k]['valueMeaningName'] : "";
                            $label = isset($result['valueDomain']['permissibleValues'][$k]['valueMeaningName']) ? $result['valueDomain']['permissibleValues'][$k]['valueMeaningName'] : "";
    ...
    /**
     * Generate UTS TGI Api Url
     * @param boolean $generate_new
     * @return void
     */
    public function generateTGIApiUrl($generate_new = false) {
        // Step 1 :: Set api key values - get this API key from UTS profile [use of Username/password is deprecated]
        $tgtApiParam['apikey'] = $GLOBALS['fieldbank_nih_cde_key'];

        // Step 2 :: API request to get a Ticket-Granting Ticket (TGT)
        if (!isset($_COOKIE['tgi_api_url']) || $generate_new == true) {
            $responseHTML = http_post($this->utsApiUrl, $tgtApiParam, 30, 'application/x-www-form-urlencoded');

            $doc = new DOMDocument;
            libxml_use_internal_errors(true);

            if ($doc->loadHtml($responseHTML)) {
                ...
                // Get the first item
                $form = $result->item(0);
                // Get the value of the action attribute and store in cookie for next time usage
                // expires after 8 hours
                if ($form !== null) {
                    savecookie('tgi_api_url', $form->getAttribute('action'), 60*60*8);
                    $_COOKIE['tgi_api_url'] = $form->getAttribute('action');
                }
            }
        }
    }
    ...
    /**
     * Generate UTS ticket per request to pass as parameter to singleCDE API call
     * @return string
     */
    public function generateUTSServiceTicket() {
        // Step 3 :: API request to get a Service ticket
        $ticketApiParam['service'] = $this->ticketService;
        $ticket = http_post($_COOKIE['tgi_api_url'], $ticketApiParam, "", 'application/x-www-form-urlencoded');
        if (empty($ticket)) {
            $this->generateTGIApiUrl(true);
            $ticket = $this->generateUTSServiceTicket();
        }
        return $ticket;
    }
