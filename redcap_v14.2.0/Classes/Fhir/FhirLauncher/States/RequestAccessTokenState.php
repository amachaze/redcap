...
class RequestAccessTokenState extends State
{
	...
	public function run() {
		...
		$this->context->log("auth code has been received"); // diagram step
		
		$this->context->log("redeem auth code for access token"); // diagram step
		$session->accessToken = $accessTokenDTO = $this->getAccessToken($fhirSystem, $code);
		$this->context->log("access token has been received"); // diagram step
	...
	public function getAccessToken($fhirSystem, $code) {
		...
		$this->context->log('token bearer was created using client ID and client secret');
		$this->context->log(sprintf("sending authorization code to 'FHIR token URL' ($URL):\n%s", json_encode($accessTokenRequestDTO->getData(), JSON_PRETTY_PRINT)));
