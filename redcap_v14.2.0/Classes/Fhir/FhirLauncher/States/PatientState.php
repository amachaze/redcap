...
class PatientState extends State
{
	...
	public function run() {
		...
		if(!$patientData || $patientData->ID !== $patientID) {
			$this->context->log("retrieving data for patient ID $patientID");	
			$patientData = $this->getPatientData($patientID, $user_id, $accessTokenDTO->access_token);
			$session->patientData = $patientData;
		}
		
		$patientMrn = $patientData->MRN;
		$this->context->log("getting registered projects with MRN '$patientMrn' for user '$user_id'");
		$registeredProjects = $this->getRegisteredProjects($patientMrn, $user_id);
		$this->context->log("getting unregistered projects for user '$user_id'");
		$unregisteredProjects = $this->getUnegisteredProjects($user_id);
		$this->context->log("getting MRN validation types");
		...
		$html = $main = $renderer->run('patient', compact('navbar','patientData', 'registeredProjects','unregisteredProjects','mrnValidationTypes'));
		print($html);
	}
