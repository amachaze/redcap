...
class FhirLauncher
{
	...
	public function detectState() {
		// order is important
 		if($_GET[self::FLAG_ERROR] ?? false) { $state = new ErrorState($this); }
		// debug screen
		else if($_GET[self::FLAG_DEBUG] ?? false) { $state = new DebugState($this); }
		// make sure there is a state
		else if(!isset($_GET[self::FLAG_SESSION_ID]) ?? false) { $state = new NoState($this); }

		else if($_GET[self::FLAG_LOGIN] ?? false) { $state = new LoginState($this); }
		else if($_GET[self::FLAG_STORE_TOKEN] ?? false) { $state = new StoreTokenState($this); }
		else if($_GET[self::FLAG_MAP_EHR_USER] ?? false) { $state = new MapEhrUserState($this); }
		...
		else if($_GET[self::FLAG_PATIENT_ID] ?? false) $state = new PatientState($this);
