...
class SurveyScheduler
{
                ...
                foreach ($this->record_data as $record => $record_data) {
                    ...
                    if (empty($funcArgArray)) {
                        ...
                        try {
                            ...
                            $parser = new LogicParser();
                            $logicThisItem = Piping::pipeSpecialTags($this->schedules[$survey_id][$event_id]['condition_logic'], $this->project_id, $record, $event_id, $instanceNum, null, true, null, $Proj->surveys[$survey_id]['form_name'], false, false, false, true);
	...
	public function renderConditionalInviteSetupTable($survey_id, $event_id)
	{
		...
		$html = // Instructions
				...
				RCView::table(array('cellspacing'=>'0','border'=>'0','style'=>'table-layout:fixed;'),
					RCView::tr('',
						RCView::td(array('valign'=>'top','style'=>'width:700px;padding:6px 10px 0 0;'),
	...
	public function checkConditionsOfRecordToSchedule($survey_id, $event_id, $record, $instanceNum=1)
	{
				...
				|| ($thisSchedule['condition_andor'] == 'AND' && $conditionsPassedSurveyComplete)))
		{
			...
			if (Piping::containsSpecialTags($thisSchedule['condition_logic'])) {
				...
				$thisSchedule['condition_logic'] = Piping::pipeSpecialTags($thisSchedule['condition_logic'], $this->project_id, $record, $event_id, $instanceNum, null, true, null, $form, false, false, false, true);
			}
			// Does the logic evaluate as true?
	...
	public static function emailInvitations($ssq_id=null)
	{
        ...
        if ($ssq_id == null)
        {
            ...
            $sql = "update redcap_surveys_scheduler_recurrence set status = 'IDLE', next_send_time = null 
            ...
            $ssr_ids = array();
            $sql = "select distinct r.ssr_id from redcap_surveys_scheduler_recurrence r, redcap_surveys_scheduler a, redcap_surveys s, redcap_projects p
                    where r.ss_id = a.ss_id and a.survey_id = s.survey_id and s.project_id = p.project_id
                    and a.active = 1 and p.status <= 1 and p.date_deleted is null and p.completed_time is null and r.status = 'IDLE'
            ...
            if (db_num_rows($q) > 0) {
                ...
                while ($row = db_fetch_assoc($q)) {
                    ...
                    $sql = "update redcap_surveys_scheduler_recurrence r, redcap_surveys_scheduler a set 
                            r.status = 'SENDING',
                            ...
                            where r.ss_id = a.ss_id and r.ssr_id = {$row['ssr_id']} and r.status = 'IDLE'";
                    db_query($sql);
            ...
            if (!empty($ssr_ids)) {
                ...
                $sql = "select a.ss_id, r.ssr_id, a.survey_id, a.event_id, r.record, s.project_id, 
                        ...
                        from redcap_surveys_scheduler_recurrence r, redcap_surveys_scheduler a, redcap_surveys s
                        where r.ss_id = a.ss_id and a.survey_id = s.survey_id and r.next_send_time is not null and r.ssr_id in (" . prep_implode($ssr_ids) . ")
                ...
                while ($row = db_fetch_assoc($q)) {
                    ...
                    $sql = "select ifnull(max(q.instance),0)+1 as nextinstance
                            from redcap_surveys_scheduler_recurrence r, redcap_surveys_scheduler_queue q
                            where r.ss_id = {$row['ss_id']} and r.ss_id = q.ss_id and q.record = '" . db_escape($row['record']) . "'";
                    $q2 = db_query($sql);
                    ...
                    if (Survey::isResponseCompleted($row['survey_id'], $row['record'], $row['event_id'], $instanceNum)) {
                        ...
                        $sqli = "insert into redcap_surveys_scheduler_queue (ss_id, record, instance, status) 
                        ...
                        $sql = "update redcap_surveys_scheduler_recurrence
                                set status = 'IDLE', times_sent = times_sent+1
                        ...
                        if ($wasSent) {
                            $sql = "update redcap_surveys_scheduler_recurrence
                                    set status = 'IDLE', times_sent = times_sent+1, last_sent = '" . NOW . "'
                                    where ssr_id = " . $row['ssr_id'];
                        } else {
                            $sql = "update redcap_surveys_scheduler_recurrence set status = 'IDLE' where ssr_id = " . $row['ssr_id'];
                        }
                    }
			...
			while ($row = db_fetch_assoc($q))
			{
					...
					|| $row['delivery_type'] == 'VOICE_INITIATE' || $row['delivery_type'] == 'SMS_INVITE_RECEIVE_CALL')
				{
					...
					if (!($twilio_enabled && ($GLOBALS['twilio_enabled_global'] || $GLOBALS['mosio_enabled_global']))) {
						...
						if ($success === true) {
							...
							db_query("insert into redcap_twilio_error_log (ssq_id, error_message) values ($ssq_id, '".db_escape($success)."')");
						}
					} else {
						## VOICE DELIVERY
				...
				else
				{
					...
					if ($participantEmail != null)
					{
						...
						$email = new Message($project_id_by_ssq_id[$row['ssq_id']], $row['record'], ($row['event_id'] ?? null), ($row['form_name']??null), ($row['instance']??null));
						...
						if (!$invitationSent && $email->emailApiConnectionError) {
							...
							$sql = "update redcap_surveys_scheduler_queue set status = 'QUEUED'
									where ssq_id = $ssq_id";
