...
class DataEntry
{
			...
			***************************************************************/
			...
			foreach ($Proj->forms as $form_name=>$attr)
			{
				...
				foreach ($form_events as $this_event_id=>$eattr)
				{
                    ...
                    $formsAccess = FormDisplayLogic::getAccess('record_home_page', $_GET['id'], $this_event_id);
                    $form_enabled = $form_enabled || ($formsAccess[$_GET["id"]][$this_event_id][$form_name] ?? null);
					// Find collapsed state of column
					...
					foreach ($grid_form_status[$row['event_id']][$row['form_name']] as $this_instance=>$this_form_status) 
					{
						...
						if (in_array($row['form_name'], $eattr))
						{
                            ...
                            // Form Display Logic
                            $link_style = "";
							if (isset($formsAccess[$_GET['id']][$row['event_id']][$row['form_name']]) && $formsAccess[$_GET['id']][$row['event_id']][$row['form_name']] != 1) {
    ...
    public static function renderForm($elements, $element_data=array(), $hideFields=array())
    {
        ...
        if ((PAGE == "DataEntry/index.php") && isset($_GET['id']))
        {
            ...
            if ($wholeRecordIsLocked) {
                ...
                $wholelock = RCView::tt_i("form_renderer_54", array(
                        $whole_lock_user,
                        "{$wholelock_user_info['user_firstname']} {$wholelock_user_info['user_lastname']}",
        ...
        elseif (PAGE == "Design/online_designer.php" || PAGE == "Design/online_designer_render_fields.php")
        {
            ...
            if (isset($_GET['page']))
            {
                ...
                $orig1 = array("{name}", "{rr_type}", "{branching_logic}", "{display_stopactions}", "{signature}", "{field_embed}", "{field_embed_container}");
                ...
                $surveyActionDropdown =
                        RCView::span(array(),
                            RCView::button(array('id'=>'SurveyActionDropDown', 'onclick'=>$surveyActionDropdownOnclick, 'class'=>"jqbuttonmed $surveyActionDropdownOpacity"),
                                RCView::img(array('src'=>'blog_arrow.png', 'style'=>'position:relative;top:-1px;margin-left:2px;')) .
        ...
        foreach ($elements as $rr_key=>$rr_array)
        {
            ...
            if ($isFormOrSurveyPage && $name != $Proj->table_pk)
            {
                ...
                if (in_array('@CHARLIMIT', $rr_array['action_tags'])) {
                    ...
                    if (is_numeric($actionTagLimitGoal) && $actionTagLimitGoal > 0) {
                        $wcl_msg = RCView::tt_js("data_entry_404");
                    ...
                    if (is_numeric($actionTagLimitGoal) && $actionTagLimitGoal > 0) {
                        $wcl_msg = RCView::tt_js("data_entry_403");
                        print  "<script type='text/javascript'>$(function(){ 
            ...
            if (PAGE == "Design/online_designer.php" || PAGE == "Design/online_designer_render_fields.php")
            {
                ...
                if ($rr_type == "header") {
                    ...
                    $fieldIcons =  '<span class="od-field-icons"><a href="javascript:;" onclick="openAddQuesForm(\'{name}\',\'{rr_type}\',0,\'{signature}\');"
                                    ><img src="'.APP_PATH_IMAGES.'pencil.png" ignore="Yes" data-rc-lang-attrs="title=global_27" title="'.RCView::tt_js("global_27").'"></a>&nbsp;
                                    <a href="javascript:;" onClick="openLogicBuilder(\'{name}\')"
                                    ><img src="'.APP_PATH_IMAGES.'arrow_branch_side.png" ignore="Yes" data-rc-lang-attrs="title=database_mods_74" title="'.RCView::tt_js("database_mods_74").'"/></a>&nbsp;
                                    <a href="javascript:;" onClick="copyField(\'{name}\')"
                                    ><img src="'.APP_PATH_IMAGES.'page_copy.png" ignore="Yes" data-rc-lang-attrs="title=asi_017" title="'.RCView::tt_js("asi_017").'" class="REDCapDesignFieldsOption"/></a>&nbsp;
                                    <a href="javascript:;" onClick="moveField(\'{name}\',\'\')"
                                    ><img src="'.APP_PATH_IMAGES.'file_move.png" ignore="Yes" data-rc-lang-attrs="title=design_172" title="'.RCView::tt_js("design_172").'" class="REDCapDesignFieldsOption"/></a>&nbsp;
                                    <a href="javascript:;" onClick="setStopActions(\'{name}\')"
                                    ><img style="display:{display_stopactions};margin-right:4px;" src="'.APP_PATH_IMAGES.'stopsign.gif" ignore="Yes" data-rc-lang-attrs="title=design_958" title="'.RCView::tt_js("design_958").'"/></a
                                    ><a href="javascript:;" onClick="deleteField(\'{name}\',0,'.$contains_par_info.');"
                                    ><img src="'.APP_PATH_IMAGES.'cross.png" ignore="Yes" data-rc-lang-attrs="title=design_959" title="'.RCView::tt_js("design_959").'" class="REDCapDesignFieldsOption"/></a></span>
                                    &nbsp;&nbsp;&nbsp;
                                    <span class="designVarName">
                                        <i>'.RCView::tt("form_renderer_16").'</i> <span data-kind="variable-name">{name}</span>
                                        {branching_logic}
                                        {field_embed}
                ...
                if ($rr_type == "select") {
                    if (in_array($name, $sql_fields)) {
                        $repl1 = array($name, "sql", $rr_array['branching_logic'], $displayStopAction, (isset($rr_array['validation']) && $rr_array['validation'] == 'signature' ? '1' : '0'),
                                        (isset($rr_array['field_embed']) ? $rr_array['field_embed'] : ''), (isset($rr_array['field_embed_container']) ? $rr_array['field_embed_container'] : ''));
                    }
                }
            ...
            if ($name != '' && $rr_type != 'descriptive') {
                $label = "<label class='fl' id='label-$name'>$label</label>";
                ...
                case 'descriptive':
                    ...
                    if (isinteger($edoc_id))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // Inline PDF
                                if (!empty(trim($rr_array["label"]))) print RCView::div(["class" => "desc-inline-spacer"], ""); // Do not add spacer if there is no label content
                                $object_unique_id = Files::getFileUniqueId();
                                print "<object data-file-id=\"$object_unique_id\" data='$image_view_page&doc_id_hash=".Files::docIdHash($edoc_id)."&id=$edoc_id&instance={$_GET['instance']}".($isSurveyPage ? "&s=".$_GET['s'] : "")."' type='application/pdf' style='height:500px;width:100%;'><iframe src='$image_view_page&doc_id_hash=".Files::docIdHash($edoc_id)."&id=$edoc_id&instance={$_GET['instance']}".($isSurveyPage ? "&s=".$_GET['s'] : "")."' style='height:500px;width:100%;border:none;'></iframe></object>";
                            }
                    ...
                    elseif ($enable_field_attachment_video_url && $video_url != '')
                    {
                        ...
                        list ($unknown_video_service, $video_url_formatted) = self::formatVideoUrl($video_url);
                        // MLM
                        $video_data_mlm = "data-mlm-field=\"$mlm_field_name\" data-mlm-type=\"video_url\" data-mlm-unknown-video-service=\"$unknown_video_service\"";
                        // Inline display
                        ...
                        // Display inside popup
                            ...
                            $video_data_mlm .= " data-mlm-video-height=\"0\"";
                            print  "<div $video_data_mlm class='div_embed_video'>
                                        <button ignore='Yes' class='btn btn-xs btn-defaultrc fs13' onclick=\"openEmbedVideoDlg('".js_escape($video_url_formatted)."',$unknown_video_service,'$name');return false;\">
                                      ...
                                      $response_id=null, $callSaveRecordHook=false, $surveyJustCompleted=false)
    {
        ...
        if (!$deletingFormOrEvent)
        {
            ...
            foreach ($_POST as $field_name=>$value)
            {
                ...
                if (strpos($field_name, "-") === false && $field_name != 'hidden_edit_flag' && $field_name != '__old_id__' && !(substr($field_name,0,10) == '_checkbox_' && $value == 'on'))
                {
                    ...
                    if (in_array($Proj->metadata[$field_name]['element_type'], ['text', 'textarea']) && $Proj->metadata[$field_name]['misc'] !== null
        ...
        {
		    Survey::saveSurveyPdfToField(PROJECT_ID, $fetched, $_GET['event_id'], $_GET['page'], $_GET['instance']);
        }
    ...
    public static function render_context_msg($custom_record_label,$context_msg)
    {
        ...
        if ($longitudinal)
        {
            ...
            $context_msg .= "<div class='yellow'>
                             ...
                             ".RCView::tt("bottom_23")." 
                             <span style='font-weight:bold;color:#800000;' data-mlm='event-name' data-mlm-name='{$_GET["event_id"]}'>".RCView::escape(strip_tags($event_name))."</span>
                             $instanceNum
                             </div>";
        }
...
//                        order by rl.record_int desc limit 1";
    ...
    public static function formatVideoUrl($video_url) {
        ...
        return [$unknown_video_service, $video_url_formatted];
    }
