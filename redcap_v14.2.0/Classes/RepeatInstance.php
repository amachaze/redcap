			...
			foreach ($Proj->eventInfo as $this_event_id=>$attr) {
				...
				if (isset($Proj->eventsForms[$this_event_id])) {
					foreach ($Proj->eventsForms[$this_event_id] as $form) {
                        ...
                        $checkboxMyCapDisabled = ($selectedValue == 'PARTIAL' && in_array($this_event_id, $schedules) ) ? 'disabled' : '';
                        $myCapClass = in_array($this_event_id, $schedules) ? " mycap-disabled" : "";
                        if ($myCapClass != '') {
                            $myCapDisableOptionClass = " whole-option-disable";
						...
						$eventForms .= RCView::div(array('class' => "clearfix"),
							RCView::div(array('class' => "repeat_event_form_div"),
								RCView::checkbox(array('name' => "repeat_form-$this_event_id-$form", 'class' => 'repeat_form_chkbox'.$myCapClass, $checkboxDisabled => $checkboxDisabled, $checkboxMyCapDisabled => $checkboxMyCapDisabled, $checkboxChecked => $checkboxChecked)) .
