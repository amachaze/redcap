...
class System
{
	...
	public static $recommendedPhpVersions = array("7.4", "8.0", "8.1", "8.2");
	...
	public static function init()
	{
		...
		define('ALLOWED_TAGS', '<col><colgroup><strike><style><code><video><audio><source><caption><canvas><ol><ul><li><label><pre><p><a><br><center><font><b><i><u><h6><h5><h4><h3><h2><h1><hr><table><tbody><tr><th><td><thead><tfoot><img><span><div><em><strong><acronym><sub><sup><map><area>');
		// Set error handler
	...
	private static function initExternalModules()
	{
		...
		{
			defined("APP_URL_EXTMOD_LIB") or define("APP_URL_EXTMOD_LIB", "https://redcap.vanderbilt.edu/consortium/modules/");
...
//                    print_array(db_fetch_assoc($q));
	...
	public static function pseudoInsertQueryParameters($sql, $params){
		$parts = explode('?', $sql);
    ...
    public static function getForeignKeyByCol($table, $col, $refcol=null)
    {
        $sql = "select constraint_name, column_name, referenced_column_name from information_schema.KEY_COLUMN_USAGE 
				where CONSTRAINT_SCHEMA = ? and TABLE_NAME = ? and referenced_column_name is not null";
        $q = db_query($sql, [$GLOBALS['db'], $table]);
        while ($row = db_fetch_assoc($q)) {
