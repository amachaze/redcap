				...
				foreach ($response_data_array as $item_key=>$this_item) {
					...
					foreach ($this_mapping as $this_event_id=>$event_array) {
						foreach ($event_array as $this_rc_field=>$rc_field_array) {
							...
							do {
								// Make sure value is normalized before it is stored in the database
								$current_item['value'] = $dbDataNormalizer->normalize($current_item['value']);
								...
								// try a smaller size if the encrypted value is too big to be stored
								if($tooBig) $dbDataNormalizer->reduceMaxSize();
							} while($tooBig);
