...
class DataExport
{
		...
		print   RCView::tr(array(),
					...
					RCView::td(array('class'=>'header', 'colspan'=>3, 'style'=>'padding:5px 10px;'),
						RCView::textarea(array('name'=>'description', 'id'=>'description', 'class'=>'x-form-field notesbox mceEditor',
							'style'=>'font-size:12px;width:99%;'), $report['description'])
					)
				);
                                    ...
                                    $usedForSmartChart=false, $returnOnlyFileFields=false, $cacheManager=null, $cacheOptions=null)
	{
		...
		if (($isDataExportAction && isset($user_rights['forms_export']) && !empty($user_rights['forms_export']) && !SUPER_USER)
	...
	public static function collapseReportFolder($folder_id, $collapse=0, $section = '')
	{
		...
		if ($collapse) {
			UIState::saveUIStateValue(PROJECT_ID, $obj, $folder_id, '1');
		} else {
			UIState::removeUIStateValue(PROJECT_ID, $obj, $folder_id);
		}
	}
	...
	public static function outputReportFoldersTable($section = '')
	{
		...
		if (count($folders) > 0)
		{
			...
			foreach ($folders as $folder_id=>$folder_name) {
                $editSaveFn = ($isProjDashboard ? "editDashFolderSave($folder_id);" : "editFolderSave($folder_id);");
                $deleteFolderFn = ($isProjDashboard ? "deleteDashFolder($folder_id);" : "deleteFolder($folder_id);");
	...
	public static function outputReportFoldersTableAssign($folder_id, $hide_assigned=0, $section='')
	{
		...
		if (count($list) > 0)
		{
			...
			{
                $id = ($isProjDashboard ? $attr['dash_id'] : $attr['report_id']);
                ...
                $onClickRfFn = ($isProjDashboard ? "dfAssignSingle($folder_id,$id,this.checked);" : "rfAssignSingle($folder_id,$id,this.checked);");
            ...
            $onClickFn = ($isProjDashboard ? "checkAllDashboardFolders($folder_id, '$ids');" : "checkAllReportFolders($folder_id, '$ids');");
			$folderTableRowsHdr = RCView::tr(array(),
									...
									RCView::td(array('class'=>'header'),
                                        ($isProjDashboard ? RCView::tt('dash_138') : RCView::tt('reporting_58'))
									)
								);
			// Add to table
	...
	public static function outputReportFoldersDropdown($section = '')
	{
		...
		$folders = self::getReportFolders(PROJECT_ID, $section);
		$onChangeFn = $isProjDashboard ? 'updateDashFolderTableAssign(this.value);' : 'updateReportFolderTableAssign(this.value);';
	...
	public static function outputReportFoldersDialog($section = '')
	{
        ...
        $onKeyPressFn = ($isProjDashboard ? 'return checkDashboardFolderNameSubmit(event);' : 'return checkReportFolderNameSubmit(event);');
        $onClickFn = ($isProjDashboard ? 'newDashboardFolder();' : 'newReportFolder();');
						...
						RCView::tr(array(),
						  ...
						  RCView::tr(array(),
							RCView::td(array('class'=>'labelrc create_rprt_hdr', 'colspan'=>3, 'style'=>'padding:5px;'),
								RCView::div(array('style'=>'color:#444;float:left;font-weight:normal;margin-top:10px;'), ($isProjDashboard ? RCView::tt('dash_135') : RCView::tt('reporting_56'))) .
        ...
        $onClickFn = ($isProjDashboard ? 'hideAssignedDashboardFolders();' : 'hideAssignedReportFolders();');
        ...
        $divId = ($isProjDashboard ? 'dash_folders_assign' : 'report_folders_assign');
			...
			RCView::div(array('class'=>'addFieldMatrixRowHdr', 'style'=>'float:left; margin-left:25px;width:440px;'),
				RCView::table(array('class'=>'form_border', 'style'=>'width:97%;'),
					RCView::tr(array(),
						RCView::td(array('class'=>'labelrc create_rprt_hdr', 'colspan'=>3, 'style'=>'color:#008000;padding:0;background:#fff;border:0;'),
							RCView::div(array('style'=>'position:relative;top:13px;background-color:#ddd;border:1px solid #ccc;border-bottom:1px solid #ddd;float:left;padding:8px 8px;'),
                                ($isProjDashboard ? RCView::tt('dash_136') : RCView::tt('reporting_55'))
							)
						)
					  ) .
					  RCView::tr(array(),
							...
							RCView::table(array(),
								RCView::tr(array(),
									...
									RCView::td(array('class'=>'nowrap', 'style'=>'padding-top:7px'),
										...
										RCView::span(array('style'=>'font-size:11px; color:#000;font-weight:normal;'), ($isProjDashboard ? RCView::tt('dash_137') : RCView::tt('reporting_57')))
									)
								)
							)
						)
		...
		$popup_content = RCView::div(array('style'=>''),
                            ($isProjDashboard ? RCView::tt('dash_134') : RCView::tt('reporting_59'))
						) .
						RCView::table(array(),
