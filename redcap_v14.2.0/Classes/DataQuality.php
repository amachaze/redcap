...
class DataQuality
{
	...
	private function executePredefinedRule($rule_id, $rule_record, $dag_discrep, $is_export=false)
	{
...
//                    }
					...
					$getDataParams = array('project_id'=>PROJECT_ID, 'records'=>($rule_record != '' ? $rule_record : array_keys($field_data_missing)),
			...
			case 'pd-7':
                ...
                $branching_params = array('records'=>($rule_record == '' ? array() : $rule_record), 'fields'=>array_merge(array_keys($branching_fields_utilized), array($Proj->table_pk)),
			...
			case 'pd-10':
                ...
                if ($rule_record != '') {
					...
					$recordBatches = array_chunk(Records::getRecordList(PROJECT_ID, $user_rights['group_id'], true), $batchSize);
                } else {
	...
	private function loadDagRecords()
	{
		...
		$sql = "select record, value from ".\Records::getDataTable(PROJECT_ID)." where project_id = " . PROJECT_ID . " and field_name = '__GROUPID__'";
		$q = db_query($sql);
		while ($row = db_fetch_assoc($q))
		{
	...
	public function executeRule($rule_id, $rule_record=null, $is_export=false)
	{
		...
		{
			$this->executePredefinedRule($rule_id, $rule_record, $dag_discrep, $is_export);
			return;
		}
...
//		}
	...
	private function loadRulesTable()
	{
		...
		foreach ($this->getRules() as $rule_id=>$rule_attr)
		{
			...
			$rulesTableData[$counter][] = ($user_rights['data_quality_execute'] ? "<div id='ruleexe_{$rule_id}' class='exebtn' style='height:28px;vertical-align:middle;'><button style='vertical-align:middle;' onclick=\"preExecuteRulesAjax('$rule_id',0);\">{$lang['dataqueries_80']}</button></div>" : "");
	...
	public function displayRulesTable()
	{
		...
		if ($user_rights['data_quality_execute'])
		{
			...
			$title .=  "<div style='float:right;padding:2px 0 0;text-align:right;font-weight:bold;'>
							...
							<button class='execRuleBtn btn btn-xs btn-rcgreen fs12 ms-2' onclick=\"$('#dqRuleRecord').prop('disabled',true);$(this).prop('disabled',true);$('#dq_results').html('');preExecuteRulesAjax(rule_ids,0);\">{$lang['dashboard_12']}</button>
							<button class='execRuleBtn btn btn-xs btn-rcgreen fs12 ms-1' onclick=\"$('#dqRuleRecord').prop('disabled',true);$(this).prop('disabled',true);$('#dq_results').html('');preExecuteRulesAjax(rule_ids_excludeAB,0,1);\">{$lang['dataqueries_116']}</button>
							".(count($this->rules) == count($this->predefined_rules) ? "" : "<button class='execRuleBtn btn btn-xs btn-rcgreen fs12 ms-1' onclick=\"$('#dqRuleRecord').prop('disabled',true);$(this).prop('disabled',true);$('#dq_results').html('');preExecuteRulesAjax(rule_ids_user_defined,0);\">{$lang['dataqueries_117']}</button>")."
						...
						<div style='float:right;font-weight:normal;color:#333;'>
							...
							<span id='dqRuleRecordParent'>
								".Records::renderRecordListAutocompleteDropdown(PROJECT_ID, true, 5000, 'dqRuleRecord',
                                    "x-form-text x-form-field", "max-width:300px;font-size:12px;margin-left:2px;", "", $lang['esignature_14'], $lang['alerts_205'])."
							</span>
						</div>";
		}
