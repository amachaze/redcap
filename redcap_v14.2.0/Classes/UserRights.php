...
class UserRights
{
	...
	public $page_rights = array(
		...
		"EmailLoggingController:index"=>"user_rights",
		"EmailLoggingController:search"=>"user_rights",
		"EmailLoggingController:view"=>"user_rights",
		"EmailLoggingController:optin"=>"user_rights",
		"EmailLoggingController:resend"=>"user_rights",
		// DAGs
	...
	private function getSuperUserPrivileges()
	{
		...
		$user_rights = array('username'=>(defined("USERID") ? USERID : ""), 'expiration'=>'', 'group_id'=>'', 'role_id'=>'',
							 ...
							 'data_export_tool'=>1, 'data_export_instruments'=>1, 'data_import_tool'=>1, 'data_comparison_tool'=>1, 'data_logging'=>1, 'file_repository'=>1,
	...
	{
		global $Proj;
		if (!defined("USERID") || !(isset($Proj->project) && is_array($Proj->project))) return $user_rights;
	...
	private function checkPageLevelPrivileges()
	{
        ...
        // If user has readonly rights to the User Rights page, disallow them from using all EmailLoggingController endpoints
        // and all UserRights folder endpoints (excluding UserRights/index.php)
        if (defined("PAGE") && $user_rights['user_rights'] == '2' && PAGE != 'UserRights/index.php' && (starts_with(PAGE, "EmailLoggingController:") || starts_with(PAGE, "UserRights/"))) {
            return false;
        }
	...
	public static function renderUserRightsRolesTable()
	{
		...
		$html .= RCView::div(array('id'=>'userClickTooltip', 'class'=>'tooltip4left','style'=>'position:absolute;padding-left:30px;'),
					RCView::div(array('class'=>'font-weight-bold fs13 mb-2'), $lang['rights_172']) .
					...
					RCView::div(array('id'=>'tooltipBtnSetCustom', 'style'=>'padding-bottom:4px;'),
						RCView::button(array('class'=>'btn btn-xs btn-defaultrc fs13 text-primaryrc', 'onclick'=>"openAddUserPopup( $('#tooltipHiddenUsername').val());"),
                            '<i class="fas fa-user-edit"></i> '.$lang['rights_153']
                        )
					) .
					// Assign User button
					RCView::div(array('id'=>'tooltipBtnAssignRole', 'style'=>'padding-bottom:4px;'),
						RCView::button(array('id'=>'assignUserBtn2', 'class'=>'btn btn-xs btn-defaultrc fs13 text-successrc'),
                            '<i class="fas fa-user-tag"></i> '.$lang['rights_156'] .
					...
					RCView::div(array('id'=>'tooltipBtnReassignRole', 'style'=>'padding-bottom:4px;'),
						RCView::button(array('id'=>'assignUserBtn3', 'class'=>'btn btn-xs btn-defaultrc fs13 nowrap text-primaryrc'),
							'<i class="fas fa-user-tag"></i> '.$lang['rights_173'] .
							RCView::img(array('src'=>'arrow_state_grey_expanded.png', 'style'=>'margin-left:5px;'))
						)
					) .
                    ...
                    RCView::div(array('id'=>'tooltipBtnRemoveRole', 'style'=>'padding-bottom:4px;'),
                        RCView::button(array('class'=>'btn btn-xs btn-defaultrc fs13 text-dangerrc', 'onclick'=>"assignUserRole( $('#tooltipHiddenUsername').val(),0)"),
                            '<i class="fas fa-user-times"></i> '.$lang['rights_175']
                        )
                    ) .
	...
	public static function renderNavigateToPageWidget($project = false) {
		...
		$widget = '
			...
			<input type="text" id="goto-page-input" class="x-form-text x-form-field fs11" style="'.$width.'" autocomplete="off" placeholder="'.$placeholder.'" title="'.$title.'" onkeydown="if(event.keyCode==13){REDCapGotoPage.goto($(this).val(), event.ctrlKey);return false;}">
		';
		...
		END);
		return $pre.$widget.$post.$styles.$script;
	}
	...
	public static function addRole($Proj, $role_name, $user)
	{
		...
		'{$_POST['data_logging']}', '{$_POST['file_repository']}', '".($_POST['double_data'] ?? "0")."', '{$_POST['user_rights']}',
...
//            }
			...
			if (isset($this_user['forms_export']) && !empty($this_user['forms_export'])) {
				// Parse the forms
				$these_forms = array();
