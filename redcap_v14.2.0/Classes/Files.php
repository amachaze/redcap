                            ...
                            if ($GLOBALS['cache_storage_system'] == 'file' && $GLOBALS['cache_files_filesystem_path'] == '' && self::isRapidRetrievalCacheFile($value))
                            {
                                ...
                                if (isinteger($pid)) {
                                    ...
                                    if ($last_logged_event != null) {
                                        // Use file creation time instead of modification time because cache files use the modification time as TTL
                                        $creation_time_unix = filectime(APP_PATH_TEMP . $value);
                                        // If file is older than its project's last_logged_event, delete it
                                        if (is_numeric($creation_time_unix) && $creation_time_unix < strtotime($last_logged_event)) {
            ...
            if ($GLOBALS['cache_storage_system'] == 'file' && $GLOBALS['cache_files_filesystem_path'] != '')
            {
                ...
                foreach ($files as $value) {
                    ...
                    if (isinteger($pid)) {
                        ...
                        if ($last_logged_event != null) {
                            // Use file creation time instead of modification time because cache files use the modification time as TTL
                            $creation_time_unix = filectime($cacheDir . $value);
                            // If file is older than its project's last_logged_event, delete it
                            if (is_numeric($creation_time_unix) && $creation_time_unix < strtotime($last_logged_event)) {
