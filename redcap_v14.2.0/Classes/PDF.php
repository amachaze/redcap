...
class PDF
{
	...
	public static function output($survey_mode = false, $hideAllHiddenAndHiddenSurveyActionTagFields = false)
	{
		...
		// Render the PDF
		PDF::renderPDF($metadata, $acknowledgement, $project_name, $data, isset($_GET['compact']), $pdf_translations, $deidFieldsToRemove ?? null);
	}
	...
	public static function confidentialText($pdf)
	{
		...
		if (PDF::$survey_mode || !(isset($Proj->project['pdf_custom_header_text']) && $Proj->project['pdf_custom_header_text'] !== null)) {
			return $pdf;
		}
	 ...
	 */
	public static function renderPDF($metadata, $acknowledgement, $project_name = "", $Data = array(), $compactDisplay=false, $translations = array(), $deid_fields = array())
	{
		global $Proj, $table_pk, $table_pk_label, $longitudinal, $surveys_enabled,
			   $salt, $__SALT__, $user_rights, $ProjMetadata, $ProjForms, $project_encoding, $context;
						...
						foreach ($nattr2 as $repeat_instance=>$dataNormalized)
						{
									...
									if ($this_string != '')
									{
										if ($record != '') {
											...
											$this_string = Piping::replaceVariablesInLabel($this_string, $record, $event_id, $repeat_instance, $piping_record_data, true, null, false, "", 1,
																							false, true, $attr['form_name'], null, false, false, false, true);
											...
											do {
												...
												$this_string2 = Piping::replaceEmbedVariablesInLabel($this_string2, $Proj->project_id, $attr['form_name'], $pdfHasData, !$pdfHasData);
								...
								// foreach ($this_metadata as $row)
							{
                                ...
                                if ($row['misc'] !== null && strpos($row['misc'], "@IF") !== false) {
                                    $row['misc'] = Form::replaceIfActionTag($row['misc'], $Proj->project_id, $record, $event_id, $attr['form_name'], $repeat_instance);
                                }

								// @HIDDEN-PDF action tags
								if (Form::hasHiddenPdfActionTag($row['misc'], $Proj->project_id, $record, $event_id, $attr['form_name'], $repeat_instance)) {
									continue;
								}

								// If a survey respondent is downloading the PDF, hide all fields with @HIDDEN and @HIDDEN-SURVEY action tags
								if (PDF::$hideAllHiddenAndHiddenSurveyActionTagFields && Form::hasHiddenOrHiddenSurveyActionTag($row['misc'], $Proj->project_id, $record, $event_id, $attr['form_name'], $repeat_instance)) {
									continue;
								}
								...
								if ($last_form != $row['form_name']) // || $last_repeat_instrument != $repeat_instrument || $last_repeat_instance != $repeat_instance)
								{
									...
									if (isset($_GET['appendEconsentFooter']) && isset($Proj->forms[$row["form_name"]]["survey_id"])
									&& $Proj->surveys[$Proj->forms[$row["form_name"]]['survey_id']]['pdf_auto_archive'] == '2')
									{
										list ($nameDobText, $versionText, $typeText) = Survey::getEconsentOptionsData($Proj->project_id, $_GET['id'], $row["form_name"]);
									...
									if (isset($translations[$row["form_name"]])) {
										$Proj->project["pdf_custom_header_text"] = $translations[$row["form_name"]]["__PDF_CUSTOM_HEADER_TEXT"];
										if (isset($_GET["appendToHeader"])) {
											$_GET["appendToHeader"] = $translations[$row["form_name"]]["__PDF_CUSTOM_HEADER_TEXT"];
										} 
								...
								if ($record != '')
								{
										...
										&& $dataNormalized[$row['field_name']] != ''))
									{
										...
										if (isset($branchingLogicValid[$row['field_name']])) {
											...
											$row['branching_logic'] = Piping::pipeSpecialTags($row['branching_logic'], $Proj->project_id, $record, $event_id, $repeat_instance, null, true, null, $row['form_name'], true, false, false, true);
										...
										if (!defined("PROJECT_ID")) {
											...
											if ($row['edoc_display_img']) {
                                                ...
                                                if (getFileExt(strtolower($fname)) == 'pdf') {
                                                    if ($GLOBALS['display_inline_pdf_in_pdf'] == '1' && class_exists('Imagick')) {
                                                        $pdf_to_image = true;
                                                    } else {
                                                        $display_inline_image = false;
                                                        $this_string .= "\n\n[Attachment: $fnameDisplay]";
                                                    }
                                                }
                                            } else {
                                                $this_string .= "\n\n[Attachment: $fnameDisplay]";
