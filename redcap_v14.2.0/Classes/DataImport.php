...
class DataImport
{
	...
	public static function storeAsyncDataRows($data, $delimiter, $date_format, $overwrite_behavior, $force_auto_number, $import_filename, $change_reason)
	{
		...
		$sql = "insert into redcap_data_import (project_id, user_id, upload_time, csv_header, delimiter, date_format, 
                overwrite_behavior, force_auto_number, filename, change_reason) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$params = [PROJECT_ID, UI_ID, date("Y-m-d H:i:s"), $csvHeaders, ($delimiter == "\t" ? "TAB" : $delimiter), $date_format, $overwrite_behavior, $force_auto_number, $import_filename, $change_reason];
	...
	public static function checkCompleteAsyncDataImport($import_id, $records_provided)
	{
		...
		if ($totalRecordsProcessed == $records_provided)
		{
			...
			if ($q && db_affected_rows() > 0)
			{
				...
				REDCap::email($row['user_email'], $GLOBALS['project_contact_email'], $subject, $message, '', '', $GLOBALS['project_contact_name']);
				return true;
			}
		}
		return false;
		...
		if (db_num_rows($q) > 0)
		{
			...
			foreach ($row_ids as $key=>$row_id)
            {
                $sql = "select i.import_id, r.row_id, i.project_id, u.username, i.csv_header, i.delimiter, i.date_format, i.status, i.change_reason,
                        i.force_auto_number, r.row_data, r.record_provided, r.event_id, i.records_provided, i.overwrite_behavior, r.row_status
                ...
                // Get error count
                $numErrors = is_array($dataSuccess['errors']) ? count($dataSuccess['errors']) : 1; // 'errors' should always be an array, so if not, there must be an error (set it to "1")
	...
	public static function loadBackgroundImportsTable()
	{
		...
		while ($row = db_fetch_assoc($q))
		{
            ...
            if ($status == 'PROCESSING' || $status == 'QUEUED') {
                ...
                $errorDisplay .= RCView::div(['class'=>'mt-1'], RCView::button(['class'=>'btn btn-xs fs11 btn-rcred nowrap px-1 py-0', 'onclick'=>"cancelBgImport($import_id);"], $lang['data_import_tool_356']));
					...
					<div class='darkgreen' style='max-width:850px;padding:20px;'>
						<div id='uploadmain'>
							...
							<div style='padding-bottom:8px;'>
								<b>{$lang['data_import_tool_231']}</b><br>
			...
			print  "<br><form action='$this_file' method='POST' id='form' name='form' enctype='multipart/form-data'>
					<div class='darkgreen' style='max-width:850px;padding:20px;'>
						<div id='uploadmain'>
							...
							"<div style='padding-bottom:15px;'>
								<div class='d-inline-block' style='width:260px;'>
									<b><i class=\"fa-solid fa-arrow-down-1-9 fs15\"></i> {$lang['data_import_tool_326']}</b>
								</div>
								<select name='forceAutoNumber' class='x-form-text x-form-field fs12' style='max-width:500px;'>
									<option value='0' ".((!isset($_REQUEST['forceAutoNumber']) || $_REQUEST['forceAutoNumber'] == '0') ? "selected" : "").">{$lang['data_import_tool_266']}</option>
									<option value='1' ".((isset($_REQUEST['forceAutoNumber']) && $_REQUEST['forceAutoNumber'] == '1') ? "selected" : "").">{$lang['data_import_tool_267']}</option>
								</select>
								<a href='javascript:;' class='help' data-bs-toggle=\"popover\" data-bs-placement=\"right\" data-content=\"".js_escape2(RCView::b($lang['data_import_tool_265'])."<br><br>".$lang['data_import_tool_273'])."\" data-title=\"".js_escape2($lang['data_import_tool_326'])."\">?</a>
							</div>"
							)."
	...
	public static function renderBackgroundImportResults()
	{
        addLangToJS(['data_import_tool_346','data_import_tool_347','data_import_tool_348','data_import_tool_349','data_import_tool_350','data_import_tool_351','data_import_tool_352',
                     'data_import_tool_353','data_import_tool_354','data_import_tool_356','data_import_tool_357','data_import_tool_359','data_import_tool_366','data_import_tool_367',
                     'data_import_tool_368','global_79','data_import_tool_369','data_import_tool_360','email_users_112','data_import_tool_374']);
