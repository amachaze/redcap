...
class ProjectDesigner
{
	...
	/**
	 * list of available responses fir the "delete field" process
	 */
	...
	const DELETE_FIELD_RELOAD_TABLE = 3; // javascriptr should reload table
	 ...
	 * @return void
	 */
	function createField($form_name, $fieldParams=[], $next_field_name='', $was_section_header=false, $grid_name='', $add_form_name=NULL, $add_before_after=NULL, $add_form_place='')
	{
		$project = $this->project;
		$project_id = $project->project_id;
		$status = $this->getProjectStatus();
		...
		// Logging
		if ($q) {
			Logging::logEvent($sql,$metadata_table,"MANAGE",$field_name,"field_name = '{$field_name}'","Create project field");
		} else {
