...
class Survey
{
                                              ...
                                              $addEmailOption=true, $project_id=null, $returnAllTwilioOptions=false)
	{
		...
		if (isinteger($project_id)) {
			...
			global $twilio_enabled, $twilio_option_voice_initiate, $twilio_option_sms_initiate,
	...
	{
		global $Proj, $pdf_econsent_system_enabled, $pdf_auto_archive, $pdf_econsent_system_ip;
		$pdf_custom_header_text = $GLOBALS["pdf_custom_header_text"];
		...
		if ($pdf_econsent_system_enabled && $pdf_auto_archive == 2) {
			...
			if ($versionText != '')	$versionTypeTextArray[] = (empty($lang_id) ? RCView::getLangStringByKey("data_entry_428") : MultiLanguage::getUITranslation($context, "data_entry_428"))." ".$versionText; //= Version:
			if ($typeText != '') 	$versionTypeTextArray[] = (empty($lang_id) ? RCView::getLangStringByKey("data_entry_429") : MultiLanguage::getUITranslation($context, "data_entry_429"))." ".$typeText; //= Type:
            ...
            if ($nameDobFilename == '_') $nameDobFilename = "";
		}
		// Format the name+dob+pid+record+form to put in the PDF filename
		...
		$pdf_contents = REDCap::getPDF($record, $form, $event_id, false, $instance, true, $pdf_custom_header_text, $versionTypeText, false, true);
	...
	{
		global $hide_back_button, $isTablet, $isMobileDevice, $isIOS, $pdf_custom_header_text;
        ...
        // iPads are hard to detect, so use JS to set height. Otherwise, the inline PDF is squished on iPads.
        $onloadJS = "<script>$(function(){ if(isIpad) $('iframe:first, object:first').height(700); });</script>";
		...
		print  "<div style='margin:20px;font-size:14px;'>".RCView::tt("survey_1169")."</div>
				<div style='margin:5px;padding:3px;border:1px solid #ccc;'>
					".($isIOS || $isMobileDevice
                        ? "<div class='p-2'>".RCView::tt("data_entry_569")." 
                            <button class='btn btn-xs btn-defaultrc ms-2 fs14' style='color:#C00000;' onclick='window.open(\"$pdfUrl\", \"_blank\");'><i class='far fa-file-pdf'></i> ".RCView::tt("survey_1364")."</button>
                            </div>"
                        : ("<object data='$pdfUrl' type='application/pdf' style='width:100%;height:$height;' onload='showProgress(0,0);'>
                            <iframe src='$pdfUrl' style='border:none;width:100%;height:$height;'>".RCView::tt("survey_1366")." <a target='_blank' href='$pdfUrl'>".RCView::tt("survey_1341")."</a>
                            </iframe>
                            </object>
                            <div class='p-2'>".RCView::tt("data_entry_606")." 
                            <button class='btn btn-xs btn-defaultrc ms-2 fs14' style='color:#C00000;' onclick='window.open(\"$pdfUrl\", \"_blank\");'><i class='far fa-file-pdf'></i> ".RCView::tt("survey_1364")."</button>
                            </div>")
                    )."
				</div>
	...
	{
		global $text_to_speech, $font_family, $text_size, $theme, $custom_theme_attr, $deleteSurveyResponse;
		...
		if ($justCompletedSurvey) {
			...
			// Store a completed survey response as a PDF in the File Repository
			if ($pdf_auto_archive > 0 && !isset($_GET['__endpublicsurvey']) && !$deleteSurveyResponse) {
...
//                    where d.record is null and r.response_id is null";
	...
	public static function sendSurveyConfirmationEmail($survey_id, $event_id, $record, $respondent_email_override=null, $instance=1)
	{
		...
		$usingEconsentFramework = ($survey_attr['pdf_auto_archive'] == '2');
			...
			if (!empty($respondent_email))
			{
				...
				if ($survey_attr['confirmation_email_attach_pdf'] == '1') {
					...
					if ($usingEconsentFramework) {
						...
						$typeFilename = str_replace(" ", "_", trim(preg_replace("/[^0-9a-zA-Z- ]/", "", $typeText))) . "_";
						if ($typeFilename == '_') $typeFilename = "";
						$pdf_filename_tmp = APP_PATH_TEMP . "ConsentForm_RecordID_" . $record . "_" . $typeFilename . date('Y-m-d_His') . ".pdf";
				...
				$emailSentSuccessfully = $email->send(false, true, $enforceProtectedEmail, null, $lang_id);
	...
	public static function saveSurveyPdfToField($project_id, $record, $event_id, $instrument, $instance)
	{
		global $pdf_econsent_system_enabled;
        $Proj = new Project($project_id);
        // Has this survey been set to save a PDF to a field?
        $survey_id = $Proj->forms[$instrument]['survey_id'];
        $fieldToSavePdf = $Proj->surveys[$survey_id]['pdf_save_to_field'];
		$formToSavePdf = (isset($Proj->metadata[$fieldToSavePdf]) ? $Proj->metadata[$fieldToSavePdf]['form_name'] : "");
        $eventIdToSavePdf = $Proj->surveys[$survey_id]['pdf_save_to_event_id'];
		$translateToSavePdf = $Proj->surveys[$survey_id]['pdf_save_translated'] == "1";
		...
		// Update context with language
		$context = Context::Builder($context)->lang_id($lang_id)->Build();
        // If survey is using eConsent Framework, add the extra eConsent footer values
		$econsentEnabled = ($Proj->surveys[$survey_id]['pdf_auto_archive'] == '2');
		$versionTypeText = "";
		if ($econsentEnabled) {
			// For eConsent, get eConsent Options data
			$nameDobText = $versionText = $typeText = $nameDobFilename = "";
			if ($pdf_econsent_system_enabled && $Proj->surveys[$survey_id]['pdf_auto_archive'] == 2) {
				list ($nameDobText, $versionText, $typeText) = self::getEconsentOptionsData($project_id, $record, $instrument);
				$versionTypeTextArray = array();
				if ($nameDobText != '')	$versionTypeTextArray[] = $nameDobText;
		...
		$pdf_content = REDCap::getPDF($record, $instrument, $event_id, false, $instance, $econsentEnabled, $pdf_custom_header_text, $versionTypeText, false, true, false);
		file_put_contents($full_path_to_temp_file, $pdf_content);
        // Store PDF in edocs
		$_FILE['type'] = "application/pdf";
		$_FILE['name'] = camelCase(strip_tags(html_entity_decode($Proj->surveys[$survey_id]['title'], ENT_QUOTES))). "_" . date('Y-m-d_Hi') . ".pdf";
        ...
        // Attach the PDF to the record/event/field/instance
