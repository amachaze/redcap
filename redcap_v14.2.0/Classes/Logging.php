...
class Logging
{
	...
	public static function setUserActivityTimestamp()
	{
		...
		if ($user_id == "" || strpos($user_id, "[") !== false) return;
    ...
    public static function getFilterHTML($getArr, $isApi = false) {
        ...
        $all_users = array();
        $previous_users = array('', 'ADMIN');

        //Call rights table for current users
        $sql = "select u.username, concat(i.user_firstname, ' ', i.user_lastname) as name 
                from redcap_user_rights u, redcap_user_information i where u.project_id = $project_id and u.username != '' and u.username = i.username";
        $q = db_query($sql);
        while ($row = db_fetch_array($q)) {
            $this_user = strtolower(trim($row['username']));
            $this_name = trim($row['name']);
            $all_users[$this_user] = $this_name;
            $previous_users[] = $this_user;
        }

        //Call log_event table for past users
        $sql = "select distinct user from ".Logging::getLogEventTable($project_id)." where project_id = $project_id and user not in (".prep_implode($previous_users).")";
        $q = db_query($sql);
        while ($row = db_fetch_array($q)) {
            if (strpos($row['user'], "SYSTEM\n") !== false) {
                list ($user1, $user2) = explode("\n", $row['user'], 2);
                $all_users[$user1] = "";
                if (!isset($all_users[$user2])) {
                    $userInfo = User::getUserInfo($user2);
                    $all_users[$user2] = trim($userInfo['user_firstname']." ".$userInfo['user_lastname']);
                }
            } else {
                $all_users[$row['user']] = "";
            }
        ...
        $tr_filter_logging .= "<tr>
                                    ...
                                    <td>
                                        <span>
                                            <input autocomplete='off' type='text' onfocus=\"$(this).next('img').trigger('click');\" ";
        if ($isApi == false) {
