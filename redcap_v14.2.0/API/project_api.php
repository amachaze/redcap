...
if (method_exists("ExternalModules\\ExternalModules", "getEnabledApiActions")) {
	...
	if (!empty($api_actions)) {
		$rows = [
			...
			RCView::span(["style" => "font-weight: normal;"], RCView::tt("api_221")),
			[
				RCView::tt("api_219"), //= Module / Action (Access)
				RCView::tt("api_220"), //= Prefix / Description
			]
		];
		foreach ($api_actions as $prefix => $module_data) {
			$rows[] = [
				RCView::b([], $module_data["name"]),
				RCView::i([], $prefix)
			];
			foreach ($module_data["actions"] as $action) {
				$rows[] = [
					$action["name"]."&nbsp;&nbsp;<i style='color:gray;'>({$action["access"]})</i>",
					$action["desc"] == "" 
						? ("<i style='color:gray;'>".RCView::tt("api_222")."</i>") //= (No description provided)
						: $action["desc"],
				];
			}		}
		$module_api_actions = RCView::div(
			...
			RCView::simpleGrid($rows, [200,  300])
		);
	}
}
