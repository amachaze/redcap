...
if (isset($_POST['copyof']) && is_numeric($_POST['copyof']))
{
	...
	while ($row = db_fetch_assoc($q))
	{
		$sql = "insert into redcap_surveys (project_id, form_name, title, instructions, acknowledgement, question_by_section,
				...
				pdf_econsent_lastname_field, pdf_econsent_lastname_event_id, pdf_econsent_dob_field, pdf_econsent_dob_event_id, pdf_save_to_field, pdf_save_to_event_id,
				pdf_econsent_signature_field1, pdf_econsent_signature_field2, pdf_econsent_signature_field3, pdf_econsent_signature_field4, pdf_econsent_signature_field5, pdf_econsent_allow_edit,
                ...
                survey_btn_text_submit, survey_btn_hide_submit, survey_btn_hide_submit_logic
				...
				".checkNull($row['pdf_save_to_field']).", ".checkNull($eventid_translate[$row['pdf_save_to_event_id']] ?? "").", 
				".checkNull($row['pdf_econsent_signature_field1']).", 
				".checkNull($row['pdf_econsent_signature_field2']).", 
				".checkNull($row['pdf_econsent_signature_field3']).", 
				".checkNull($row['pdf_econsent_signature_field4']).", 
				".checkNull($row['pdf_econsent_signature_field5']).", 
				".checkNull($row['pdf_econsent_allow_edit']).", 
        ...
        while ($row = db_fetch_assoc($q)) {
            $sql = "INSERT INTO redcap_mycap_tasks (project_id, form_name, enabled_for_mycap, task_title, question_format,
                    ...
                    VALUES
                        ...
                        {$row['allow_retro_completion']}, {$row['allow_save_complete_later']}, {$row['include_instruction_step']}, {$row['include_completion_step']},
                        " . checkNull($row['instruction_step_title']) . ", " . checkNull($row['instruction_step_content']) . ", " . checkNull($row['completion_step_title']) . ", 
                        " . checkNull($row['completion_step_content']) . ", " . checkNull($row['schedule_relative_to']) . ", " . checkNull($row['schedule_type']) . ", 
                        " . checkNull($row['schedule_frequency']) . ", " . checkNull($row['schedule_interval_week']) . ", " . checkNull($row['schedule_days_of_the_week']) . ", 
                        " . checkNull($row['schedule_interval_month']) . ", " . checkNull($row['schedule_days_of_the_month']) . ", " . checkNull($row['schedule_days_fixed']) . ", 
                        " . checkNull($row['schedule_relative_offset']) . ", " . checkNull($row['schedule_ends']) . ", " . checkNull($row['schedule_end_count']) . ", 
                        " . checkNull($row['schedule_end_after_days']) . ", " . checkNull($row['schedule_end_date']) . ", " . checkNull($row['extended_config_json']) . ")";
            db_query($sql);
            ...
            $q2 = db_query($sql2);
            while ($row2 = db_fetch_assoc($q2)) {
                $sql_schedule = "INSERT INTO redcap_mycap_tasks_schedules (task_id, event_id) 
                                VALUES ($this_task_id, ".$eventid_translate[$row2['event_id']].")";
	...
	if ($isTemplate || (isset($_POST['copy_survey_queue_auto_invites']) && $_POST['copy_survey_queue_auto_invites'] == "on"))
	{
		...
		while ($row = db_fetch_assoc($q))
		{
			$sql = "insert into redcap_surveys_scheduler (survey_id, event_id, instance, active, email_subject, email_content, email_sender, email_sender_display,
					...
					reminder_nextday_type, reminder_nexttime, reminder_exact_time, reminder_num, reeval_before_send, condition_send_time_lag_field, condition_send_time_lag_field_after) values
...
if (isset($_POST['copyof']) && is_numeric($_POST['copyof'])) {
    ...
    if ($templateMyCapEnabled || (isset($_POST['copy_mycap_mobile_app_content']) && $_POST['copy_mycap_mobile_app_content'] == "on"))
    {
        ...
        $myCap->copyProjectParticipantSettings($copyof_project_id, $new_project_id);
        if ($addedToMyCapProjects) {
