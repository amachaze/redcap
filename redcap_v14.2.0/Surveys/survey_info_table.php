...
<form id="survey_settings" action="<?php echo $_SERVER['REQUEST_URI'] . ((isset($_GET['redirectInvite']) && $_GET['redirectInvite']) ? "&redirectInvite=1" : "") ?>" method="post" enctype="multipart/form-data">
	<table cellspacing="3" style="width:100%;">
		...
		<tr>
			<td colspan="3">
				<div id="survey_enabled_div" class="clearfix <?php echo($survey_enabled ? 'darkgreen' : 'red') ?>" style="max-width:1020px;margin: -5px -7px 0px;font-size:13px;">
        ...
        <tr>
            <td valign="top" style="width:25px;padding-top:8px;">
                <i class="far fa-file-pdf fs16" style="color:#D00000;"></i>
            </td>
            <td valign="top" style="width:290px;font-weight:bold;padding-bottom:20px;padding-top:5px;">
				<?php echo $lang['survey_1303'] ?>
                <div style="font-weight:normal;">
                    <i><?php echo $lang['survey_1304'] ?></i>
                </div>
            </td>
            <td valign="top" style="padding-left:15px;padding-bottom:20px;padding-top:5px;">
				...
				print RCView::select(array('name'=>'pdf_save_to_event_id', 'class'=>'x-form-text x-form-field me-1', 'style'=>'max-width:150px;'.($longitudinal ? "" : "display:none;")),
					    $pdfSaveEvents, (isset($pdf_save_to_event_id) ? $pdf_save_to_event_id : ''), 300);
				print RCView::select(array('name'=>'pdf_save_to_field', 'class'=>'x-form-text x-form-field', 'style'=>'max-width:350px;'),
					    Form::getFieldDropdownOptions(false, false, false, false, [''], true, true, false, 'file', $lang['global_23']), (isset($pdf_save_to_field) ? $pdf_save_to_field : ''), 300);
				?>
				<script>
					$(function() {
						$('select[name=pdf_save_to_field]').on('change', function() {
							if ($(this).val() == '') {
								$('input[name=pdf_save_translated]').prop('checked', false);
								$('#pdf_save_translated-div').addClass('opacity35');
							}
							else {
								$('#pdf_save_translated-div').removeClass('opacity35');
							}
						});
					});
				</script>
				<?php
				// Translate the PDF?
				$translate_pdf_checked = $pdf_save_translated == "1" ? "checked" : "";
				$translate_pdf_opacity = empty($pdf_save_to_field) ? "opacity35" : "";
				print 	RCView::div(array('id'=>'pdf_save_translated-div', 'style'=>'margin:8px 0 5px;color:#333;', 'class'=>$translate_pdf_opacity),
							RCView::checkbox(array('name'=>'pdf_save_translated', $translate_pdf_checked=>$translate_pdf_checked)) .
							RCView::span(['class'=>'text-dangerrc'], $lang['survey_1370']) .
                            RCView::span(['class'=>'fs11 ms-1'], $lang['survey_1371'])
						);
				?>
                <div class="cc_info" style="margin-top: 10px;">
                    <?php echo $lang['survey_1488']; ?>
                    <a href="javascript:;" onclick="simpleDialog(null,null,'survey-specific-email-dialog',600);" style="font-size:11px;text-decoration:underline;"><?php echo $lang['alerts_32'] ?></a>
                </div>
                <div class="simpleDialog" title="<?=RCView::tt_js2('survey_1303')?>" id="survey-specific-email-dialog">
                    <?=RCView::tt('survey_1489')?>
                    <?php if ($Proj->hasRepeatingFormsEvents()) { ?><div style="margin-top: 10px;"><?php echo $lang['survey_1307']; ?></div><?php } ?>
                </div>
            </td>
        </tr>
        ...
        <tr>
            <td colspan="3" style="padding-bottom:20px;font-weight:bold;font-size:13px;color:#B00000;">
                ...
                <img src="<?php echo APP_PATH_IMAGES ?>stopsign.gif"> <?php echo $lang['survey_1324'] ?>
            </td>
        </tr>
        <tr>
		...
		<tr>
			<td valign="top" colspan="2" style="width:310px;font-weight:bold;padding-bottom:5px;padding-left:5px;">
                ...
                <?php } ?>
                <div class="text-primaryrc">
                    <i class="far fa-file-pdf fs16 me-1"></i> <?php echo $lang['survey_1157'] ?>
                </div>
			...
			<td valign="top" style="padding-left:15px;padding-bottom:5px;">
				...
				<?php if ($pdf_econsent_system_enabled) { ?>
				<div>
					<input type="radio" name="pdf_auto_archive" value="2" onclick="checkBackBtn();showHidePDFcompactLabel(1);$('#pdf_econsent_allow_edit').prop('checked',true);" <?php echo ($pdf_auto_archive == '2' ? 'checked' : '') ?>>
				...
				<div id="pdf_econsent_options" style="padding:5px 8px;background-color:#f5f5f5;border:1px solid #ddd;font-size:12px;margin:8px 0 0;<?php echo ($pdf_auto_archive == '2' ? '' : 'display:none;') ?>">
					<div class="text-dangerrc" style="margin-bottom:8px;line-height:14px;">
					...
					<div style="margin:8px 0 4px;padding-top:7px;border-top:1px dashed #ccc;font-size:12px;line-height:13px;color:#C00000;">
                    ...
                    for ($sn=1; $sn<=5; $sn++) {
                        ...
                        $sigFieldHtml .= RCView::div(array('class' => 'my-1 pdf_econsent_signature_field_div'),
                            ...
                            RCView::select(array('name' => $this_pdf_econsent_signature_field, 'class' => 'x-form-text x-form-field', 'style' => 'margin:0 3px 0 10px;max-width:200px;'),
                                $sigDDoptions, $GLOBALS[$this_pdf_econsent_signature_field] ?? "")
                        );
                    }
                    ?>
                    <div style="margin:8px 0 4px;padding-top:7px;border-top:1px dashed #ccc;font-size:12px;line-height:14px;color:#C00000;">
