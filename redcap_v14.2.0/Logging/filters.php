...
if ($_GET['usr'] == 'SYSTEM') {
    $filter_user = (isset($_GET['usr']) && $_GET['usr'] != '') ? "AND (user = 'SYSTEM' or user like 'SYSTEM\\n%')" : "";
} else {
...
$filter_record = '';
if ($dagId === null && isset($_GET['record']) && $_GET['record'] != '') {
    // Single record filtering (user not in DAG)
    $filter_record = "AND event in ('MANAGE','ESIGNATURE','LOCK_RECORD','UPDATE','INSERT','DELETE','DOC_UPLOAD','DOC_DELETE','OTHER') 
                      and pk = '".db_escape($_GET['record'])."' and object_type != 'redcap_user_rights' and !(object_type = 'redcap_alerts' and description != 'Send alert')";
} elseif ($dagId !== null) {
    ...
    if (empty($dagRecords) && $user_rights['group_id'] == '') {
        ...
        $filter_record_dag = empty($dagRecords) ? "and pk in ('')" : "and pk in (" . prep_implode($dagRecords) . ")";
        if (strpos($logtype, 'record') === false) {
            ...
            $dag_users_sql = "";
            if (!empty($dag_users_array)) {
                $dag_users_sql = "(user in (" . prep_implode($dag_users_array) . ") and 
                                    !(event in ('MANAGE','ESIGNATURE','LOCK_RECORD','UPDATE','INSERT','DELETE','DOC_UPLOAD','DOC_DELETE','OTHER') 
                                      $filter_record_dag and object_type != 'redcap_user_rights' and !(object_type = 'redcap_alerts' and description != 'Send alert'))
                                  ) OR";
            }
            $filter_record = "AND (
                                $dag_users_sql
...
if (isset($_GET['logtype']) && $_GET['logtype'] == 'page_view') {
    if ($filter_user == '' && $filter_record == '' && $dag_users == '') {
        $logging_sql = "SELECT ts*1 as ts, user, '0' as legacy, full_url, event, page, event_id, record, form_name
					   FROM redcap_log_view_old WHERE project_id = $project_id $filter_logtype ";
        $logging_sql2 = "SELECT ts*1 as ts, user, '0' as legacy, full_url, event, page, event_id, record, form_name
					   FROM redcap_log_view WHERE project_id = $project_id $filter_logtype ";
    } else {
        $logging_sql = "SELECT ts*1 as ts, user, '0' as legacy, full_url, event, page, event_id, record, form_name
					   FROM redcap_log_view_old WHERE project_id = $project_id $filter_logtype $filter_user $dag_users ";
        $logging_sql2 = "SELECT ts*1 as ts, user, '0' as legacy, full_url, event, page, event_id, record, form_name
    ...
    if ($beginTime_YMDts != "") $logging_sql2 .= " AND ts >= '".db_escape($beginTime_YMDts)."' ";
    if ($endTime_YMDts != "") $logging_sql2 .= " AND ts <= '".db_escape($endTime_YMDts)."' ";
    $logging_sql = "($logging_sql) union ($logging_sql2) ORDER BY ts DESC";
